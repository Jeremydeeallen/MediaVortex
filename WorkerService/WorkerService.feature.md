...
20. [BUG] **Worker status is two states (Online, Paused), not three.** Today the code has three states: Online, Draining, and Paused. Draining is a transient internal state that the UI never exposes (the Activity page only shows Online and Pause buttons). The Draining handler was written before RemuxService existed and does not signal it to stop -- remux jobs keep being claimed during the drain window. The drain-to-paused auto-transition then triggers a second code path that does stop remux, adding ~120s of unnecessary delay (two poll cycles). Fixed = collapse Draining into Paused. Paused means "set StopRequested on every capability (transcode, remux, VMAF, scan), finish in-flight jobs, do not claim new work." One state, one code path, all capabilities covered. Remove `_DrainAndStop`, remove the `Draining` branch from `_HandleStatusChange`, remove the drain waiter thread. The `_StopAllCapabilities` method already knows about every service -- make Paused use it directly.
21. [BUG] **Capability polling interval and concurrency ceiling have no justification and should be removed.** The capability polling loop has a hardcoded 1-5 clamp on concurrency values (`_ApplyConcurrencyChanges`). This was already removed from the API validation and TeamStatus controller but survives in the polling loop. The 60s capability polling interval means status/concurrency changes take up to 60s to take effect -- criterion 2 says "within one polling interval (default 15s)" but the flow doc says "every 60 seconds." Fixed = (a) remove the 1-5 clamp from `_ApplyConcurrencyChanges` (floor of 1 only, no ceiling), (b) align the actual polling interval to the documented default of 15s, (c) verify `SystemSettings.CapabilityPollingIntervalSec` is actually read and applied.
