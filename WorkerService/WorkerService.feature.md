...
16. [BUG] **`MaxConcurrentJobs` from the Workers table is ignored when starting capabilities; workers always run 1 concurrent job.** `_StartTranscodeCapability()` and `_StartQualityTestCapability()` both hardcode `Run(MaxConcurrentJobs=1)` instead of reading `self.WorkerConfig['MaxConcurrentJobs']`. Setting `Workers.MaxConcurrentJobs=2` (or any value >1) in the DB has no effect -- the worker still processes one queue item at a time. Fixed = each capability reads `MaxConcurrentJobs` from the worker's config (or re-reads from DB) at start time and passes it to `Run()`, so a worker configured for N concurrent jobs actually claims and processes N items simultaneously.
