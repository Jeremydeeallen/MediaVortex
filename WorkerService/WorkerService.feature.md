...
- `WorkerService/Main.py` -- process entry point (systemd unit target, StartMediaVortex tab)
...
7. Systemd units (bare-metal Linux) and Task Scheduler (Windows) use WorkerService as their entry point.
...
10. **Crash recovery does not terminate the running worker.** When the recorded `ActiveJobs.ProcessId` matches `os.getpid()`, the cleanup logic treats the row as a stale prior-process artifact and skips the kill step (PID reuse after a systemd restart can hit the new process). Verified by running 4 workers through restart cycles with no spurious self-termination.
...
- [x] Update systemd unit ExecStart
...
- [x] Feature doc
- [x] Deployed to 4 systemd worker instances, verified transcode jobs processing correctly
