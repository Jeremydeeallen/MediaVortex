...
| ST14 | `WorkerStateReporter.Transition()` (runtime state) | The SRP writer for the worker-authored truth columns on `Workers`: `RuntimeState`, `CurrentAttemptId`. Called at every lifecycle transition: Initializing -> Idle -> ClaimingJob -> Encoding -> Idle (success) or `Faulted:<reason>` (uncaught exception, before re-raise). WebService never writes these columns. Freshness is derived from `LastHeartbeat` alone -- see `S4`. |
| ST15 | `HungEncodeDetector` sweep | Pure function classifies a worker as `IsHung=true` when `RuntimeState='Encoding'` on the same `CurrentAttemptId` for > `SystemSettings.HungEncodeThresholdSec` (default 600) AND `TranscodeProgress.LastProgressUpdate` for that attempt has not advanced in the same window. `StuckJobDetectionService` invokes the detector each sweep and auto-recovers: kills the ffmpeg subprocess by `FFmpegPid`, flips `TranscodeAttempts.Success=FALSE` with `ErrorMessage='hung_encode_detector'`, deletes the `ActiveJobs` row. Worker on its next lifecycle tick transitions back to `Idle`. |
...
| S2 | `ST2` share mappings | Env var `MEDIAVORTEX_SHARE_MAPPINGS` parsed -> UPSERT into `WorkerShareMappings` + `StorageRootResolutions` | `StorageRootResolutions.(StorageRootId, WorkerName, Platform, AbsolutePath, IsActive=TRUE)` | `Core.Path.LocalPath / Core.Path.Path.Resolve` (`Core/Path/path.feature.md::S1`) reads these rows | `SELECT AbsolutePath FROM StorageRootResolutions WHERE WorkerName=<host>` returns one row per mounted root |
...
| S4 | `ST9` heartbeat | `_StartHealthMonitoring` thread | `Workers.LastHeartbeat=NOW()` every 30s | `teamstatus.flow.md::S2` consumer + `AttemptAbandonmentSweeper` cross-worker heartbeat check | `SELECT NOW() - LastHeartbeat FROM Workers WHERE WorkerName=<host>` < 60s |
...
| S7 | `ST12` queue producers/consumers | Services started here drive the seams in `transcode.flow.md::S1` (all modes including Remux; see `transcode.flow.md ST6 Strategy variants`) | per-capability claim queries | Pending rows in `TranscodeQueue` / `QualityTestingQueue` / `RootFolders` are consumed | The cross-flow verifications listed in those seams |
| S8 | `ST14` worker-authored truth columns | `WorkerStateReporter.Transition()` -- the only writer | `Workers.(RuntimeState TEXT, CurrentAttemptId BIGINT NULL)` UPDATE | `AdminWorkersRepository.GetTiles` reads + derives `IntentDiverges` flag against `Workers.Status` when worker is FRESH (heartbeat within `HeartbeatStaleThresholdSec`); `/Admin/Workers` Truth badge renders the RuntimeState | `grep -rn 'UPDATE Workers SET .*RuntimeState\|CurrentAttemptId' Features/ WebService/` returns 0 matches; `Tests/Contract/TestWorkerRuntimeStateAuthorship.py` |
| S9 | `ST14 + ST15` hung-encode signal | `Workers.(RuntimeState, CurrentAttemptId)` UPDATE + heartbeat freshness on `Workers.LastHeartbeat` + `TranscodeProgress.LastProgressUpdate` UPDATE | `(RuntimeState='Encoding', AttemptId BIGINT, HeartbeatAge int, ProgressAge int)` joined per worker | `HungEncodeDetector` classifies; `StuckJobDetectionService` auto-recovers; `AdminWorkersRepository.GetTiles` derives `IsHung` for tile rendering; `ActivityRepository.GetHungAttempts` for the /Activity banner | `Tests/Contract/TestHungEncodeDetector.py` |
