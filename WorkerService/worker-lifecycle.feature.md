...
### End-to-End Worker Ownership (BUG-0020)

22. **[BUG-0020]** A worker that produces a `.inprogress` file owns that file's terminal state. On any non-success exit (FFmpeg failure, FFprobe verification failure, FileReplacement failure, compliance-gate refusal, kill/crash before final commit), the same worker deletes the `.inprogress` artifact and any partial sibling (e.g. unfinalized `-mv.mp4`) before releasing its `ActiveJobs` slot. No other service (OrphanCleanupService, scan adoption, sibling worker, manual scripts) is permitted to delete `.inprogress` files whose owning worker is alive (heartbeat fresh AND `ActiveJobs` row present). Crash recovery (criteria 10-13) remains the safety net for the case where the worker died before reaching its own cleanup, and operates only on rows owned by the restarting worker.
23. **[BUG-0020]** A worker that completes an encode AND succeeds at FileReplacement owns the post-replacement cleanup: TFP row removal for that attempt, `MediaFiles` row finalization, and Jellyfin notification. No other service may touch `TemporaryFilePaths` rows whose owning worker is alive. This closes the BUG-0018 race -- the sweep cannot delete TFP rows out from under a live owner because the live owner is the only deleter.

### Compliance-Gated Rename (BUG-0020)

24. **[BUG-0020]** The `.inprogress` -> final-name rename only fires after the candidate output passes the **same compliance predicate the cascade uses** to decide whether a file needs work (`Features/TranscodeQueue/QueueManagementBusinessService._EvaluateCompliance`). The worker probes the `.inprogress`, synthesizes a candidate `MediaFile`-shaped row (carrying forward source-file fields the probe cannot derive: `HasExplicitEnglishAudio`, loudness measurements, `AudioComplete`), runs `_EvaluateCompliance`, and only proceeds with the rename when `(IsCompliant, RecommendedMode) == (True, None)`. The FFprobe sanity check (criterion 7) is necessary but no longer sufficient. **A successful FFmpeg run that produces a non-compliant output is not a green light to rename.**
25. **[BUG-0020]** When the compliance gate refuses the rename, the worker emits a non-Replace disposition (`NoReplace` with `DispositionReason='ComplianceGateFailed'` and an audit payload naming the specific cascade reason -- e.g. `audio_not_normalized`, `container_not_acceptable`, `video_codec_not_acceptable`, `marginal_savings`). The `.inprogress` is deleted by the owning worker (criterion 22). No `-mv.mp4` or `-mv-mv.mp4` artifact survives a compliance-gate refusal, eliminating the generational-stacking class of bugs (BUG-0013 family).
26. **[BUG-0020]** After this slice ships, the operator-run sweeps (`Scripts/SQLScripts/CleanupSourceFileOrphans.py`, `CleanupStaleInProgressFiles.py`, `CleanupGenerationalGhostRows.py`, `CleanupOrphanMvPairs.py`) report zero candidates on a fresh fleet pass. Any non-zero candidate is a worker bug, not an expected sweep target. The sweeps remain in the repo as diagnostic tools, not as load-bearing cleanup.

## Status

PARTIAL -- criteria 1-21 COMPLETE 2026-05-16; criteria 22-26 OPEN (BUG-0020, opened 2026-05-26). Mount-validation slice (20, 21) shipped 2026-05-15; status-model collapse, `.inprogress` rewrite, crash recovery, Activity tile redesign, and polling-interval verification shipped 2026-05-16. End-to-end worker ownership + compliance-gated rename slice (22-26) is the BUG-0020 feature pass, to be folded together with BUG-0015 (disk orphans), BUG-0016 (DB ghost-row pairs), BUG-0018 (TFP sweep race) as a single "worker process ownership + compliance-gated rename" pass.
