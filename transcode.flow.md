...
Adding a new Strategy verify path is one `Strategy.Verify()` implementation + one Registry.Register line. The disposition decision is Strategy-blind: `DecidePostTranscodeDisposition` reads `VMAF` + thresholds and returns Replace/Requeue/NoReplace regardless of how the score was computed.
