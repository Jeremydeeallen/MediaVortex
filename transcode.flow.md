...
```
SCAN -> PROBE -> ASSIGN -> RECOMPUTE -> QUEUE -> TRANSCODE -> DISPOSITION -> VERIFY -> ACTION
...
- TRANSCODE -> DISPOSITION -> VERIFY or ACTION (ST6 -> ST7 -> ST8 or ST9): `Features/QualityTesting/Disposition/PostTranscodeDispositionDecider.Decide` is the single decision function -- reads `TranscodeAttempts.QualityTestRequired` + `VMAF` + `PostTranscodeGateConfig` and returns one of `Replace`/`Pending`/`Requeue`/`Reject`. `DispositionDispatcher.Dispatch` routes accordingly: `Pending` enqueues to QualityTestingQueue (ST8); any other disposition goes straight to ST9.
...
- **Compact-source classification** (`video-encoding.feature.md`): `VideoVertical.Evaluate` compares `SourceKbps` against `Tier1TargetKbps * Multiplier` where the multiplier lives in `VideoComplianceThresholds` per resolution. Sources at or below the multiplier floor land in `WorkBucket='Remux'` / `'Audio'` / `'Compliant'` at scan time instead of `'Transcode'`. Admission-time exclusion is no longer needed -- classification IS the gate.
...
| `DispositionDecidedAt` | timestamp | When the decision was committed. Distinguishes "decided" from "still Pending". |
...
## Stage 7: VERIFY -- Verify Execution (when Disposition='Pending' on first decision) (`ST8`)
...
| Operator override | WebService endpoint `POST /api/QualityTest/Override` sets `ForceDisposition='Replace'` or `'Reject'` on a queue row; WebService acts immediately | Operator decides to short-circuit (no worker available, or known-good encode, or known-bad encode) |
...
- For each pending test:
  ...
  4. Re-call `DecidePostTranscodeDisposition(TranscodeAttemptId)` -- this time the VMAF score is available, the disposition will be `Replace` / `Reject` / `Requeue` per the table
  5. UPDATE QualityTestingQueue SET Status='Completed' for this row

**Operator override path:**
- Operator POSTs `{queueId, forceDisposition: 'Replace'|'Reject', reason: 'optional note'}` to `/api/QualityTest/Override`
- WebService (no worker capability required -- has DB + share access):
  ...
  2. Write `TranscodeAttempts.Disposition='Replace'` (for Replace) or `'Reject'` (for Reject); `DispositionReason='OperatorForcedReplace'` or `'OperatorDiscarded'`
  3. If Replace: call `FileReplacementBusinessService.ProcessFileReplacement(attemptId)` synchronously
  4. If Reject: delete the `.inprogress` output and the TFP row
...
### ST8 Strategy variants -- per-strategy `Verify` implementation

ST8 has one orchestration body (`QualityTestingBusinessService.ProcessQualityTestQueue`) and per-strategy Verify implementations. `Strategy.Verify()` returns `(Score, Method)` where `Method IN {'VMAF','Checksum'}` and `Score` is a number in `[0,100]` (`100.0` = bit-identical).
...
- `DispositionDispatcher._MaybeScheduleRequeue` inserts a new `TranscodeQueue` row for the same `MediaFileId` via canonical `QueueManagementBusinessService.AddJobToQueue(ForceAdd=True)`, unless `RetryBudgetService.HasBudgetRemaining=False` (in which case Requeue folds to `Reject/RetryBudgetExhausted`).
...
`Features/TranscodeQueue/TranscodeQueueRepository.ClaimNextPendingJob(WorkerName, AcceptsInterlaced)` runs one statement: `UPDATE TranscodeQueue tq_outer SET Status='Running', ClaimedBy=$worker, ClaimedAt=NOW() WHERE tq_outer.Id = (SELECT tq.Id ... ORDER BY <priority contract> FOR UPDATE OF tq SKIP LOCKED LIMIT 1) RETURNING ...`. Applies `BuildClaimPredicate(WorkerName, ClaimCapabilityFlag)` (capability + Online gate), the NVENC/QSV EXISTS gates, and the failure-budget cap predicate. `FOR UPDATE SKIP LOCKED` = two workers racing the same row is impossible.
