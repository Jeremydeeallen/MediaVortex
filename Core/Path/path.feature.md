...
Each v1 vertical migrates by removing `Core.Path.LocalPath / Core.Path.Path` imports and routing path consumption through `Path` + `Worker`. The pattern established by the pathfinder (`mediaprobe-uses-path`, closed 2026-06-04) is the canonical recipe for the remaining six verticals: FileScanning, FileReplacement, TranscodeJob, QualityTesting, TranscodeQueue, Activity.

**Imports:** swap `from Core.Path.LocalPath / Core.Path.Path import Resolve as PathResolve, LocalExists` (or any subset) for `from Core.Path import Path, Worker, PathError`.
...
| D8 | `__str__` shape | `CanonicalDisplay()` (with no args -- reads `GetPrefixMap()` fresh per call). | Ergonomic: `f"{path}"` and logger `%s` formatters show the operator-readable canonical string. Hot-path callers that cannot afford the per-call DB read MUST call `path.CanonicalDisplay(hoisted_prefix_map)` explicitly. `repr(path)` remains the diagnostic shape (D7) for grep + logs that need stable identity output. |
...
| D10 | `FromLegacyString` validation rules | Case-insensitive prefix match against `CanonicalPrefix`; longest-prefix wins (caller responsible for sort); empty / no-match raises `PathError`. Relative tail normalized per D9. | Mirrors v1 `Core.Path.LocalPath / Core.Path.Path.Parse()` behavior but raises instead of returning `(None, None)` -- migrations fail loudly. |
...
| C19 | `Path.CanonicalDisplay(Prefixes: Optional[dict] = None)`. When `Prefixes` is omitted, the call reads `GetPrefixMap()` fresh from DB. Hot-path callers (per-row loops) hoist the map once and pass it; single-shot callers omit the parameter. Both shapes return identical output for the same `(StorageRootId, RelativePath)` and matching prefix map. |
| C20 | `str(path)` returns `path.CanonicalDisplay()` (operator-readable). `repr(path)` returns `<Path #<id>:<rel>>` (diagnostic, no DB call). f-string interpolation (`f"{path}"`), `logging.info("%s", path)`, and bare `print(path)` therefore display the canonical string by default. Verified by `Tests/Unit/test_path_repr.py::test_exact_repr_shape` + `::test_str_returns_canonical_display`. |
| C21 | `Worker.Current(Db=None)` is the canonical entry point for obtaining a Worker in service / controller code (alias for `FromWorkerContext`). Stateful services hold the Worker as instance state via a constructor-time `self._Worker` field + a `_GetWorker()` helper that lazy-loads on first read. In-method ad-hoc `Worker.FromWorkerContext()` calls inside such services are replaced by `self._GetWorker()` so all method calls on the same service share one Worker instance. Cross-cutting helpers that lack a stateful owner call `Worker.Current()` at the seam. Verified by `Tests/Unit/test_path_worker.py` (existing) + Live restart smoke (Activity page, claim/process flow). |
| C22 | HTTP-boundary controllers that accept a canonical-string path input (`ShowFolder`, `FilePath`, `RootFolderPath`) catch `PathError` and respond with HTTP 400 whose body includes the bad input AND the current `AvailableRoots` list (from `GetPrefixMap().values()`). Operators see the mismatch immediately and can correct their input without grepping logs. Applies to: `AudioCompletionController._ResolveMediaFileIds`, `MediaProbeController` filter parsing, `WorkBucketController` ShowFolders bulk-update endpoint, `SQLQueriesController` MV-pair lookup. Internal (non-HTTP) callers continue to surface PathError to their callers without wrapping. |
| C23 | `RootFolderModel.Path` is the typed accessor (returns `Path(self.StorageRootId, self.RelativePath)` or `None`). The model exposes the typed pair only. Callers wanting a canonical display string call `str(folder.Path)` or `folder.Path.CanonicalDisplay(prefix_map)` for hot paths. Templates render `folder.Path` via `Path.__str__`. |
| C24 | `Path.FromLegacyString` prefix match is case-sensitive (consistent with D2 case-sensitive Path identity). `FromLegacyString("t:\Show/x", [{"Id":2,"CanonicalPrefix":"T:\\"}])` raises `PathError("no matching prefix")`; the HTTP-boundary 400 (C22) returns the case-correct prefixes in `AvailableRoots` so operators can fix their input. |
| C25 | `Core.Path.PathStorageRoots.PrefixMapScope` context manager pins a StorageRoots snapshot for the duration of its lifetime via `contextvars`. Flask `before_request` opens a scope; `teardown_request` closes it -- every `str(Path)` / `CanonicalDisplay()` / `GetPrefixMap()` within a request reuses one DB read. Worker tick loops MAY wrap each iteration in `PrefixMapScope()` for the same benefit (opt-in; outside any scope, the accessors read DB fresh per call, preserving db-is-authority). |
| C26 | The 6 stateful services (`QualityTestQueueService`, `MediaProbeBusinessService`, `TranscodedOutputPlacement`, `FileReplacementBusinessService`, `OptimizationViewModel`, `QualityTestingBusinessService`) accept `worker: Optional[Worker] = None` in their constructors, defaulting to `Worker.Current()`. `_GetWorker()` is a one-line accessor returning `self._Worker`. Tests inject a mock Worker via the constructor; production code uses the default. `Worker.Current` is the sole entry point. |
| C27 | `RootFolderModel.__init__` accepts a typed `(StorageRootId, RelativePath)` pair only. Callers that have a canonical-display string convert it explicitly: `parsed = Path.FromLegacyString(s, GetStorageRoots())` then `RootFolderModel(StorageRootId=parsed.StorageRootId, RelativePath=parsed.RelativePath, ...)`. |
| C28 | Every FS op given a CANONICAL string (input from DB / API / config, still in `T:\...` / `Z:\...` / `\\host\share\...` shape) goes through `Core.Path.PathFs.CanonicalExists(str, worker)` or `Core.Path.PathFs.CanonicalGetSize(str, worker)`. Direct `LocalPath.LocalExists(canonical_str)` is a bug class -- on non-Windows the guard raises; on Windows it silently returns False when the local mount prefix differs from the canonical drive letter. Verified by `Tests/Contract/TestPathFsCanonicalExists.py` (per S16). |
...
| S16 | canonical string -> FS check | any caller holding a canonical-display string (`Z:\Videos\Sub`, `T:\Show\ep.mkv`, `\\host\share\...`) | `PathFs.CanonicalExists(str, worker) -> bool` / `PathFs.CanonicalGetSize(str, worker) -> int` | translates via `Path.FromLegacyString` -> `Path.Resolve(worker)` internally; sole public API for canonical-input FS ops. Direct `LocalPath.LocalExists(canonical_str)` is refused by `_AssertLocalShape` on non-Windows and returns misleading False on Windows workers whose `WorkerShareMappings.LocalMountPrefix` differs from the canonical drive letter (BUG-0098). Private per-module helpers (e.g. `FileScanning._CanonicalExists`) are 1-line delegates or deleted. | `Tests/Contract/TestPathFsCanonicalExists.py` (seeds worker with `LocalMountPrefix != DriveLetter`; asserts translation + zero drift warns) |
