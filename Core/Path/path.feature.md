...
| C23 | `RootFolderModel.Path` is a `Path` instance synthesized from `(self.StorageRootId, self.RelativePath)` -- the typed surface callers should prefer. `RootFolderModel.RootFolder` retained as a one-line back-compat property returning `str(self.Path)` (which renders via `Path.__str__` -> `CanonicalDisplay()`). Hot-path readers (multi-access-per-iteration loops, e.g. `ContinuousScanService._ExecuteScan`) hoist `.RootFolder` (or `.Path`) once per iteration to a local variable to avoid per-access DB reads. Setter on `RootFolder` continues to parse a legacy canonical string via `FromLegacyString` and populate the typed pair. |
