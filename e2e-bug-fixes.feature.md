...
### Group P -- Compliance gate stale FR guard (added 2026-07-21)

C28. `AudioVertical.Evaluate` no longer defers with `loudness_measurement_failed` when `SourceIntegratedLufs` is populated. Root cause 2026-07-21 (surfaced by e2e-bug-fixes.C27 canary): `AudioRemeasurementRunner` runs concurrently with worker encodes; when a worker's ffmpeg holds the source open (SMB read), the runner's concurrent ffmpeg exits -1 and `PersistLoudness` writes `LoudnessMeasurementFailureReason='ffmpeg_exit_4294967295'`. The persisted row now has BOTH the valid earlier `SourceIntegratedLufs` and the transient failure reason. `AudioVertical.Evaluate` checked only `LoudnessMeasurementFailureReason` -> defer -> post-encode ComplianceGate rejects with `ComplianceGateFailed: loudness_measurement_failed` even though the just-encoded output measured cleanly. Fix: guard now requires BOTH `FailureReason IS NOT NULL AND SourceIntegratedLufs IS NULL` to defer. Also bulk-cleared 3 stale rows (The Resident S01E06/S01E09/S01E11) whose FR was set while measurements were still valid. Verification: post-fix, requeue The Resident S01E11 (MediaFileId=8167) on wakko-worker-1 -> `.inprogress` renames to final; `SELECT Disposition FROM TranscodeAttempts WHERE Id = <next>` returns `Replace` not `Reject`. (Follow-up idea, not in this criterion: teach `AudioRemeasurementRunner` to skip files with active TranscodeAttempts in-flight so the race stops occurring at all.)
