...
|---|---|---|---|---|---|
| S1 | `ST1` mutation choke points | `FileReplacementBusinessService._ProcessCompleteFileReplacement` (transcode.flow.md::ST9), `FileScanningBusinessService.ReconcileWithDisk` (filescanning.flow.md::ST5), `ShowSettings` delete handler, manual operator delete endpoints | One call per mutation batch; `Updates` list shape: `[{"Path": "<absolute path in Jellyfin's index>", "UpdateType": "Created|Modified|Deleted"}]` | `NotifyJellyfin` dispatches once per call | `grep -rn "NotifyJellyfin(" Features/ WebService/ Scripts/` -- every mutation site referenced in `jellyfin-push-notify.feature.md` criterion 1 appears |
