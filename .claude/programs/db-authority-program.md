...
- All Linux hosts (larry, wakko, dot) use bare-metal systemd units with `TimeoutStopSec=30m` so systemd doesn't SIGKILL the process before drain completes.
- `deploy/deploy-fleet.py` drains before deploy by default: flips `Workers.Status='Paused'`, polls ActiveJobs until empty per worker, then restarts systemd units, then restores pre-deploy Status. `--no-drain` flag for emergencies.
