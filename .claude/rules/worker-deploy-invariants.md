...
**I3. `Workers.Version` is derived from a tree artifact stamped alongside the code.** A `VERSION` file sits at the repo root containing the sha. Deploy writes it as part of the code drop (rsync on baremetal ships it; I9 writes it before spawning WorkerService). Worker reads it once at boot and reports on every heartbeat. Consequence: once the process starts, `Workers.Version` is correct. Deploy does not need a poll-until-heartbeat verification loop -- start success + VERSION stamp before start = correct Version on next heartbeat, guaranteed.
