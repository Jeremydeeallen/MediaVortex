...
|---|---|---|---|
| R1 | Edit/Write to code requires prior Read of any same-directory `*.feature.md` / `*.flow.md`. Same-directory only -- no walk-up. **Flow-stub extension:** when the code carries `# see <flow-slug>.ST<N>`, a partial Read of the named `*.flow.md` covering ST<N> satisfies R1 instead. **Anchor-driven narrowing (2026-07-02):** when the code carries `# see <slug>.<W\|S\|C\|ST><N>` anchors, R1 gates only docs whose slug is referenced -- colocated docs the code doesn't anchor to are treated as sibling-noise. **Inline override:** `# allow: R1 <reason>` bypasses R1 for that file. **Path normalization:** relative + absolute Read paths both count (paths resolved against repo root before dedup). **Counter decay:** refusal counters for a file are cleared when a subsequent edit passes all rules -- exploration-refusals do not sticky-lock future edits | `.claude/rules/ceo-mode.md` (Documents first) + `.claude/rules/flow-docs.md` (nav hub) | `Test-R1-DocPreread` + `Test-R1FlowStubSatisfied` + `Reset-RefusalCountsForFile` |
...
| R13 | No premature `*.feature.md` / `*.flow.md` files. Creation refused outside DELIVERING phase; at DELIVERING, creation is allowed so durable content can be promoted out of the directive doc into its permanent home. **Override:** add a line under `### R13 overrides` in `.claude/directive.md` naming the path -- allows docs-first directives where feature/flow docs are the SPEC and code implements them, not the other way around | This file + `.claude/rules/doc-layering.md` + `.claude/rules/ceo-mode.md` | `Test-R13-NoNewFeatureDocs` |
...
| R19 | DatabaseManagerSteering: Edit/Write on `Repositories/DatabaseManager.py` whose edit region overlaps any `def <method>(` block produces a refusal naming the per-aggregate target repo from `.claude/standards/database-manager-aggregates.json` (longest-prefix-first; method names with no prefix match prompt the operator to add a row). Pure-deletion edits pass silently. Forward-looking steer, not a backward-looking gate: see `Core/Database/repository-split.feature.md#perfect-end-state`. R6 path-shape check is suppressed for this single file -- it's known debt being migrated, and scanning it charges rent without producing forward progress; R6 still fires on every other file (including the new per-aggregate repos). | `Core/Database/repository-split.feature.md` + this directive | `Test-R19-DatabaseManagerSteering` (+ per-file early-return inside `Test-R6-PathShape`) |
...
- **Call-graph audit (DDD+SOLID system-wide)** -- every directive at NEEDS_STANDARDS_REVIEW audits the WHOLE call graph the feature participates in, not just the layer being edited. Four signals to check: (1) multiple flow docs for one conceptual operation, (2) mode-branching at the orchestration layer (`if Job.IsRemux: ...`), (3) shared output columns sparsely populated across modes (e.g. `AudioPolicyResolved IS NULL` for an entire mode's attempts), (4) ambiguous `## Out of Scope` items (does the clause preserve behavior, or also tolerate duplication?). Findings get absorbed into scope OR explicitly named as known debt in the directive's `## Out of Scope` -- silent inheritance is refused. Policy: `.claude/rules/call-graph-audit.md`. Operationalized after the work-transcode-unified UI-only landed atop a divergent worker pipeline (2026-06-28).
