...
C8. **Partial completion: 1 of 4 smoke benchmarking scripts has hardcoded `loudnorm=I=-23:LRA=7:TP=-2` removed: `Scripts/Smoke/NewGirlEncodingABC.py`.** The other three (`EncodeAndVmaf.py`, `FourKEncodingABC.py`, `NewGirlEncodingABC_VarianceBoost.py`) hit pre-existing R1/R6 hook refusals (colocated `EncoderShootout.feature.md` not read; `os.path.exists` on path-named variables). Per scope discipline (`feedback_preexisting_bug_scope_test.md`: verification-blocking is the only free pass; loudnorm removal works independently of path-handling), these three are deferred to a follow-up directive `smoke-scripts-cleanup` that addresses R6 paths + R1 prereads + the loudnorm removals together. C4's audit test temporarily whitelists the three deferred files with the BUG reference. Verifiable: `grep -n 'loudnorm' Scripts/Smoke/NewGirlEncodingABC.py` returns zero matches; C4 audit test passes with the deferred-file whitelist documented.
