...
- **Next:** Reset 7 live smoke (b): GUI enqueue on a Remux/AudioFix-bucket candidate -> RemuxJobStrategy -> Replace; record TranscodeAttempts row + audio-emit check. Then Reset 8 -- C5 code (populate `AudioPolicyResolved` / `AudioPolicyJson` / `AudioTracksEmittedJson` for every strategy via `PostEncodeMeasurementService` extension).
- **Phase:** IMPLEMENTING
