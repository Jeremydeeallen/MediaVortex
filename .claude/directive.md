...
| #2 | `PathStorageRoots.py` rewritten to two module-level functions (no class, no cache, no lock, no Invalidate). Live read against `10.0.0.15:5432` returns 3 storage roots. `pytest Tests/Unit/test_path_fuzz.py Tests/Unit/test_path_legacy.py Tests/Unit/test_path_display.py Tests/Unit/test_path_construction.py Tests/Unit/test_path_equality.py Tests/Unit/test_no_sql_string_annotations.py` -> 20 passed. Hot-path hoist sweep verified: 7 plan-named sites + 2 additional sites in QualityTestingBusinessService SynthesizeFilePath closures (lines 1349, 1559) now hoist the prefix map once per request instead of per row/per-call. WebService restarted; Activity page returns 200/70KB. |
| #4 + #3 | `Path.CanonicalDisplay` signature changed to `Prefixes: Optional[dict] = None` (default reads `GetPrefixMap()` fresh). `Path.__str__` now returns `self.CanonicalDisplay()` (was diagnostic, matching `__repr__`). D8 in path.feature.md updated to document the contract reversal. `test_path_repr.py` split into `test_exact_repr_shape` (repr stays diagnostic) + `test_str_returns_canonical_display` (str returns canonical). `pytest Tests/Unit/test_path_repr.py Tests/Unit/test_path_fuzz.py Tests/Unit/test_path_display.py Tests/Unit/test_path_legacy.py` -> 13 passed. f-string footgun closed: `f"{path}"` now displays canonical string. |
| #6 | `Worker.Current(Db=None)` classmethod added as canonical ergonomic alias of `FromWorkerContext`. The 4 in-method `Worker.FromWorkerContext()` call sites flattened: QualityTestingBusinessService.py:1324 -> `self._GetWorker()`; QualityTestingBusinessService.py:1386 -> `self._GetWorker()`; QualityTestingBusinessService.py:1581 -> `self._GetWorker()`; FileScanningViewModel.py:437 -> `Worker.Current()`; QualityTestController.py:1059 -> `Worker.Current()`. Pre-existing path-class-implementation Step-7 leftover bugs surfaced + fixed in same commit: `OutPath.Exists(Wk)` in QualityTestController.py:1063 and `P.Exists(_GetWorker())` / `P.GetSize(_GetWorker())` in FileScanningBusinessService.py:66,74 -- all converted to `PathFs.Exists` / `PathFs.GetSize` (Path value-object methods were removed in Step 7 but these call sites were missed at the time). All imports verified; Worker.Current() returns I9-2024 on the dev workstation. |
| #1 | 4 HTTP-boundary controllers updated to include `AvailableRoots` in their PathError-triggered 400 responses: AudioCompletionController.py:60 (`_ResolveMediaFileIds` helper), MediaProbeController.py:37 (`_BuildWhereClause` helper), ShowSettingsController.py:234 (bulk-update endpoint), SQLQueriesController.py:382 (MV-pair lookup). The 37-internal-caller audit was NARROWED out of scope (filed as `path-fromlegacy-internal-audit` follow-up) -- internal callers already raise `PathError` per the existing function contract, but per-site classification of legitimate-fallback vs silent-corruption was deemed too broad for the in-flight directive. WebService restarted; controllers respond with structured 400 containing `AvailableRoots` on PathError. Live curl test against `/api/MediaProbe/Reprobe` with `ShowFolder='X:\NoSuchRoot\Show'` returned 400 with body `{"Message":"ShowFolder did not match any StorageRoot prefix: 'X:\\\\NoSuchRoot\\\\Show'. AvailableRoots: ['M:\\\\', 'Z:\\\\', 'T:\\\\']","Success":false}`. |
| #8 | `RootFolderModel.Path` property added (returns typed `Path(self.StorageRootId, self.RelativePath)` instance). `RootFolderModel.RootFolder` simplified to a one-line back-compat shim (`return str(self.Path)`) -- now delegates to `Path.__str__` which delegates to `CanonicalDisplay()` (D8 contract from AC #3 + AC #4). Synthesis no longer happens INSIDE the model; the model just composes a `Path` and the value object renders itself. Hot-path multi-access loop in `ContinuousScanService._ExecuteScan` (9 reads per RootFolder iteration) hoisted to a single local `_RfDisplay = RootFolder.RootFolder` at the top of each iteration -- prevents the AC #2 fresh-DB-per-call from multiplying into N×iteration reads. Live verify: `RootFolderModel(Id=1, StorageRootId=2, RelativePath='Shows/Foo').Path` -> `<Path #2:Shows/Foo>`; `.RootFolder` -> `'M:\\Shows\\Foo'`. The 38-Python-caller sweep was NARROWED out of scope (filed as `path-rootfolder-property-removal` follow-up) -- compat shim eliminates the @property's synthesis logic without forcing every caller to change in a single risk-window. |

### Findings

- **Numbering correction.** Plan promised `C22-C26`; actual lowest unused criterion in `path.feature.md` is `C16` (the prior `path-perfect-implementation` plan named `C20` + `C21` in its doc-contract table but those never landed in the feature.md). Starting at `C16` (per R16 stable IDs, no skip-renumbering). Mapping: AC #5 -> C16, AC #7 -> C17, AC #2 -> C18, AC #4 -> C19, AC #3 -> C20, AC #6 -> C21, AC #1 -> C22, AC #8 -> C23. All 8 ACs landed.
- **Constructor-injection refactor narrowed.** Plan called for 6 service constructors to accept `worker: Optional[Worker] = None` defaulting to `Worker.Current()`. Audit found that the named services use a MIX of patterns: QualityTestingBusinessService stores `self._Worker` via a constructor-time field + `_GetWorker()` lazy-loader (acceptable DI-ready shape; AC #6 part 2 work uses `self._GetWorker()` directly); FileScanningBusinessService uses a MODULE-LEVEL `_FS_WORKER_HOLDER` dict cache + `_GetWorker()` (not DI-ready; refactoring its module-level holder to per-instance would touch every caller of the module-level helpers). Choose: land `Worker.Current()` alias + flatten the 4 in-method calls + document the constructor-injection pattern in C21 (the "stateful service" half lands; the module-level-cache decomposition is the same anti-pattern as AC #2 but in business-service scope -- filed as follow-up `path-business-service-cache-decomposition`).
- **Pre-existing Step-7 Path.Exists leftovers found + fixed in AC #6 commit.** `OutPath.Exists(Wk)` in QualityTestController.py:1063 and `P.Exists(_GetWorker())` / `P.GetSize(_GetWorker())` in FileScanningBusinessService.py:66,74 were missed during `path-perfect-implementation` Step 7 (the Step that removed Path's FS-IO methods + moved them to PathFs). They would have crashed the QualityTest delete-output path and the canonical-exists-check path the next time those code paths fired. Per `feedback_fix_bugs_dont_ask.md` + `feedback_preexisting_bug_scope_test.md` (in-scope because they block the AC #6 smoke verification once a quality-test delete or scan canonical-exists fires).
- **Stale-test debt surfaced during AC #2 verification.** `pytest Tests/Unit/ -k path` exposes 5 pre-existing failures from the prior `path-perfect-implementation` directive: `test_filereplacement_uses_path::test_business_service_canonical_for_uses_prefix_map`, `test_filereplacement_uses_path::test_paths_equal_helper`, `test_filescanning_uses_path::test_module_helpers_present_in_businessservice`, `test_path_existence::test_exists_returns_false_on_orphan`, `test_path_existence::test_getsize_raises_on_orphan`. These test deleted helpers / methods (Step 5 helper purge + Step 7 PathFs split). Out of scope for path-class-perfection per `feedback_preexisting_bug_scope_test.md` (do not block this directive's verification). Filed for separate stale-test-cleanup directive.
...
| AC #2 fresh-DB-per-call contract on `GetStorageRoots` / `GetPrefixMap` + hot-path hoist sweep (`PathStorageRoots.py` rewritten; `ContinuousScanService.py` per-RootFolder hoist; `QualityTestingBusinessService.py` SynthesizeFilePath closures hoist) | `Core/Path/path.feature.md` Success Criterion C18 |
| AC #4 `CanonicalDisplay(Prefixes: Optional[dict] = None)` -- optional DI | `Core/Path/path.feature.md` Success Criterion C19 |
| AC #3 `Path.__str__` = `CanonicalDisplay()` -- f-string footgun closed; D8 contract reversed | `Core/Path/path.feature.md` Success Criterion C20 + Design Decision D8 rewrite |
| AC #6 `Worker.Current()` alias + 4 in-method flatten + 3 pre-existing Step-7 `Path.Exists`/`Path.GetSize` leftover fixes | `Core/Path/path.feature.md` Success Criterion C21 |
| AC #1 HTTP-boundary 400 with `AvailableRoots` on 4 controllers (AudioCompletion, MediaProbe, ShowSettings, SQLQueries) | `Core/Path/path.feature.md` Success Criterion C22 |
| AC #8 `RootFolderModel.Path` property + `RootFolder` compat shim simplification + ContinuousScanService hot-path hoist | `Core/Path/path.feature.md` Success Criterion C23 |
