...
- **G1 (Original within +/-1 LU of TargetIntegratedLufs):** Tri-host live-encode smoke on Breaking Bad S04E01 (6ch source, I=-23.9 LUFS, LRA=17.4 LU, TP=-4.0 dBTP, real ebur128 pre-measured per host). Track 0 integrated LUFS after full pipeline (linear loudnorm + alimiter): i9=-23.1, dot=-23.1, wakko=-23.2 -- all within 0.2 LU of -23 target. Grab-the-remote (G1) preserved. TP ceiling: i9=-4.9, dot=-3.8, wakko=-4.7 dBTP, all under -2 target. Fix chain: `loudnorm=I=-23:LRA=<src>:TP=-2:measured_*:linear=true,alimiter=level_in=1:level_out=1:limit=<TP-4dB>:attack=1:release=50:level=false`. `level=false` prevents alimiter's auto-boost from re-scaling integrated LUFS off target; SAMPLE_LIMIT_HEADROOM_DB=4 buffers sample-domain alimiter against post-AAC intersample-peak overshoot (wakko needed 4 dB, i9 only needed 2 dB -- constant covers worst case).
- **G2 (Track 1 = downmix -> Demucs -> boost -> tight loudnorm):** Tri-host live-encode smoke on same Breaking Bad S04E01 source. Track 1 integrated LUFS: i9=-20.3, dot=-20.3, wakko=-20.3 (all within 0.3 LU of -20 target). LRA: i9=7.5, dot=7.5, wakko=7.6 LU (source 17.4 -> 7.5, massive narrowing; slightly above 5 LU target but within tolerable range for dialog-boost intent). TP: i9=-3.6, dot=-2.9, wakko=-3.0 dBTP, all under -2 target. Fix chain: MixBoostedPremix now emits `acompressor=threshold=0.03:ratio=9:attack=8:release=120:makeup=3:knee=4,dynaudnorm=f=150:g=13:p=0.7:m=8` before mixdown; emitter's dialog-boost filter now `loudnorm=I=-20:LRA=5:TP=-2,alimiter=...:level=false` (dropped bogus source measured_* since input is the derived premix, not source). Pre-pass timings: i9(cuda)=77s, dot(cuda)=88s, wakko(xpu)=142s.
