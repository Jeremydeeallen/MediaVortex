...
- **Current step:** Reset 7 code complete -- (C3) `TranscodeQueueRepository.ClaimNextPendingJob` SQL rewritten data-driven: 4 `tq.ProcessingMode != 'Transcode' OR ...` fragments replaced with `pm.RequiresInterlacedFilter` / `pm.RequiresProfileGates` join columns; new migration `AddProcessingModesRequiresProfileGates_2026_07_03.py` backfills column (Transcode=TRUE, others=FALSE); claim UPDATE-FROM rewired to correlated subqueries (rows with NULL MediaFileId now claimable; sentinel-test infra fixtures pass without changing prod semantics). (C4) new `Features/TranscodeJob/ProcessingModeMetadata.py` mirrors ProcessingModes rows (RequiresProfileGates / SupportsFocus / WorkBucketFilterSql / DefaultProfileNameFallback / IsKnown / GetOrDefault); Strategy interface gains `DefaultProfileName(Job)`; all 5 strategies (Transcode/Remux/AudioFix/Quick/SubtitleFix) implement it. Mode-branches deleted: `JobProcessor.py:110` -> `Strategy.DefaultProfileName`; `DashboardSnapshotService._EstimateSavings` -> data-driven guards only; `TranscodedOutputPlacement.Execute` -> `RunComplianceGate: bool` param; `FileReplacementBusinessService.ProcessFileReplacement` uses `ModeMeta['RequiresProfileGates']` for size-guard + ComplianceGate; `QueueManagementBusinessService.SmartPopulateQueue/AddSuggestionsToQueue/QueueAllMatching/PopulateQueueForRemux/AddJobToQueue` 8 branches collapsed to metadata lookups. New `Tests/Contract/TestNoModeBranchingAtOrchestration.py` greps `Features/{TranscodeJob,TranscodeQueue,FileReplacement,Activity}` outside `**/Strategies/**`, `**/Models/**`, `**/PostFlightProcessors/**`, `ProcessingModeMetadata.py` for the mode-branch pattern; expects zero hits. `Tests/Contract/TestClaimAuthority.py` sentinels now insert a real MediaFile+Profile+TranscodeAttempt fixture -- all 18 tests green. Regression sweep 28/28 pass across TestClaimAuthority + TestEnqueueContract + TestAddJobToQueueForceAdd + TestNoModeBranchingAtOrchestration.
- **Next:** Reset 8 -- C5 code (populate `AudioPolicyResolved` / `AudioPolicyJson` / `AudioTracksEmittedJson` for every strategy via `PostEncodeMeasurementService` extension). Reset 7 live smokes DONE: NVENC I9-2024 attempt 41036 (Animaniacs S01E13, av1_nvenc) VMAF=89.64 Success=True Disposition=NoReplace (ComplianceGateFailed: channels_exceed_max:6>2 -- pre-existing audio pipeline issue). QSV wakko-worker-1 attempt 41040 (New Girl S06E03, av1_qsv) encode Success=True (8.5x realtime), VMAF=51.21, Disposition=Requeue (VmafBelowMin; profile `-preset 1 -b:v 480k` low-VMAF output -- profile tuning). Both hardware paths verified through data-driven `pm.RequiresProfileGates` + BuildNvencPredicate / BuildQsvPredicate. Env fixes: Workers.qsvcapable=TRUE synced on wakko-worker-1 to match deployment reality (wakko has Intel Arc B580 per `deploy/compose-templates/wakko.yml`). New follow-ups: BUG-0081 (StuckJobDetector phase-blind kills legitimate Demucs, discovered via wakko first-attempt 5-min timeout cycle; SystemSettings.FrozenProgressThresholdMin bumped 5->30 for smoke retry, restored to 5).
- **Phase:** IMPLEMENTING
- **Last commit:** (Reset 7 pending commit)
- **Follow-ups noted:**
