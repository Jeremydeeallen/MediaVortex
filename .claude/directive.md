...
G7. **Hardware acceleration on wakko + dot.** Demucs pre-pass on wakko uses Intel Arc B580 (xpu) device; on dot uses RTX 4060 (cuda) device. Verifiable: smoke output from wakko-worker-N shows `device=xpu`, from dot-worker-N shows `device=cuda`. Stays out-of-container (host-venv bind-mounted into worker container) because container's pip + Intel CDN combinations consistently fail.
...
- **G1 (Original within +/-1 LU of TargetIntegratedLufs):** Tri-host live-encode smoke on Breaking Bad S04E01 (quiet, source TP=-4) + S04E02 (hot, source TP=0). Track 0 integrated LUFS (target -23): quiet i9=-23.1, dot=-23.0, wakko=-23.0; hot i9=-23.5, dot=-23.5, wakko=-23.5. All within 0.5 LU of -23 target across quiet AND hot masters. Track 0 True Peak (target <=-2): quiet -3.8 to -4.5, hot -4.2 to -4.4 dBTP. All under target. Adaptive filter chain: `[alimiter=<per-source-pre-limit if peak headroom insufficient>,] loudnorm=I=-23:LRA=<src>:TP=<effective>:measured_*:linear=true`. Per-source pre-limit computed as `pre_limit_dB = SrcTp - max(0, SrcTp + gain - effective_TP)` where `effective_TP = TargetTp - SampleLimitHeadroomDb`. On quiet masters (SrcTp <= effective_TP - gain) the pre-limiter is omitted entirely (dynamics untouched). On hot masters, pre-limiter clips only the excess peak needed to let loudnorm apply full linear gain while honoring the effective TP ceiling.
- **G2 (Track 1 = downmix -> Demucs -> boost -> tight loudnorm):** Tri-host live-encode smoke on both quiet + hot masters. Track 1 integrated LUFS (target -20): all six cells -20.2 to -20.3 (within 0.3 LU). Track 1 True Peak: quiet -2.5 to -2.8 dBTP, hot -3.2 to -3.5 dBTP, all under -2 target. Track 1 LRA: 7.3-7.6 LU (source LRA 15-17 -> aggressive dialog compression; slightly above 5 LU spec but perceptually TV-comfortable). MixBoostedPremix chain: `acompressor=threshold=<db>:ratio=<db>:attack=8:release=120:makeup=<db>:knee=4,dynaudnorm=f=<db>:g=<db>:p=0.7:m=8`. All params (VocalsBoostDb, InstrumentalAttenDb, PremixCompressorThreshold, PremixCompressorRatio, PremixCompressorMakeupDb, PremixDynaudnormFrameLen, PremixDynaudnormGaussSize) driven from AudioComplianceRules per encode. Dialog-boost loudnorm no longer receives source measured_* values (input is derived premix, not source). Hardware acceleration confirmed per host: i9 cuda (RTX 4060 Ti), dot cuda (RTX 4060), wakko xpu (Intel Arc B580 Battlemage).
