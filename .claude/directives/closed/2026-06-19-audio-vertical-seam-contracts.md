...
- audio-normalization.feature.md `## Cross-Vertical Contract` added with 4 sub-sections (writes/reads/functions/non-contracts) + HTTP API table + cross-vertical relationships table
- audio-normalization.feature.md intra-feature Seams pruned (S1-S9 retained as genuine audio-internal; ex-S9/S10/S11 removed entirely per R14 -- they belonged to Self-Healing + Workers verticals)
- transcode.flow.md ST stage rewritten to reference the audio seam by name (`GetEffectivePolicy + Probe + EmitTracks`) instead of inlining audio internals
- Commit db07853 pushed; deployed to:
    dot       -- all 4 workers Online, version=db07853, heartbeat<30s
    larry     -- all 4 workers Online, version=db07853, heartbeat<10s
    wakko     -- deploy skipped; SSH unreachable (Pre-flight failed at 30s as designed). Wakko stays Paused at version c401ae6 until network restored.

### Promotions

| Source artifact | Target |
|---|---|
| Cross-Vertical Contract content | `Features/AudioNormalization/audio-normalization.feature.md` `## Cross-Vertical Contract` section (already promoted) |
| Audio seam reference replacement | `transcode.flow.md` ST FFmpeg-command-build stage (already promoted) |
| Intra-feature Seams pruning | `Features/AudioNormalization/audio-normalization.feature.md` `## Seams` (already promoted) |
