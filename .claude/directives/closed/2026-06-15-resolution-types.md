...
| AC10 | **(a) MF 621554 live remediation:** `POST /api/Compliance/Recompute {"MediaFileIds":[621554]}` -> `Bucketed.Transcode=1`. Enqueued (140074 prio 200) -> claimed by I9-2024 as TranscodeAttempt 37754. Live FFmpeg argv contains `-vf "scale=w=1280:h=-2"`. Encode completed in 11 min: output `1280x694` (cinematic 1.85:1 preserved), `Codec=av1`, `SizeMB=466` (was 2732 MB; **82.94% reduction**), `IsCompliant=True`, `TranscodedByMediaVortex=True`, `FileReplaced=TRUE`, `Disposition=BypassReplace/QualityTestingGloballyDisabled`. **(b) Fleet deploy:** operator-confirmed fleet rollout 2026-06-15 (push origin/main `b421e83..47bf08a`); all heartbeating workers on HEAD. **(c) Library-wide recompute:** `POST /api/Compliance/Recompute {"All":true}` -> Evaluated 50556 rows. Pre/post bucket deltas: Remux/False 11677 -> 11653 (-24); Transcode/False 20340 -> 20337 (-3); None/True 13083 -> 13100 (+17); None/None (gate-blocked) 5456 -> 5466 (+10). 27 files left a work bucket; 17 became compliant; 10 became gate-blocked under the typed classification. **(d) At-risk audit:** 227 MediaFiles with off-canonical 1080p shape (Width in [1700,1919], Height < 1080: e.g. 1914x1076 x93, 1916x1076 x36, 1918x802 x8, 1916x1040 x2). Post-recompute all 227 are correctly bucketed (220 Transcode/False ready to re-encode under the typed scale-filter path; 7 gate-blocked). | Claude / I9 | 2026-06-15 | PASS |
