...
DIRECTIVE: Build the perfect audio normalization vertical at
`Features/AudioNormalization/` owning every audio-policy decision and
exposing one seam (`AudioFilterEmitter.EmitTracks`) consumed by every
encode shape. Implement two-knob normalization, dual-track output,
4-scope settings hierarchy, admission gate + un-deferral loop, +/-4 LU
hard ceiling, vertical absorption of LoudnessAnalysis + AudioCompletion
+ DialNorm + speech-language detection scaffolding. Close the production
bleed caused by `UngainablePeakError` swallowed in `RemuxShape.Build`'s
blanket `except Exception`.

STATUS: Done (19/23 fully delivered + 4/23 partial; live-encode ffprobe
smokes pending next WorkerService restart).

WHAT SHIPPED:
- 12 new code files under `Features/AudioNormalization/` covering the full
  resolver -> classifier -> emitter -> shape pipeline, plus admission gate,
  validator, DialNorm handler, language detector (5+1 layers),
  measurement service (absorbed), operator-review service, remeasurement
  service, post-encode probe, enrichment scaffold, controller, and
  repository.
- 1 Flask blueprint + 1 tabbed Bootstrap template at
  `/AudioNormalization` (Settings / Dashboard / Review).
- 4 PostgreSQL migrations (`AudioNormalizationConfig` table,
  `TranscodeAttempts.AudioPolicyJson` + `AudioTracksEmittedJson`,
  `MediaFiles.AudioStreamLanguageDetectionsJson`,
  `TranscodeQueue.AudioPolicyJson`) + 1 view
  (`v_audio_consistency_summary`) + 1 global default seed row.
- 1 library-wide sweep script.
- 9 new contract test suites (116 tests, all green).
- 3 shape rewires (`RemuxShape`, `TranscodeShape`, `SubtitleFixShape`) +
  composition root update + 3 shape test fixture updates.
- 5 deletions (`AudioFilterBuilder.py`, `UngainablePeakError.py`,
  `TestAudioFilterBuilder.py`, `TestUngainablePeakError.py`,
  `TestLinearLoudnormEnforcement.py`, `LoudnessAnalysisService.py`,
  `BackfillLoudnessThreshold.py`) + `Features/LoudnessAnalysis/` dir
  removed.
- 1 promoted feature doc (`audio-normalization.feature.md`).

HOW TO USE IT:
- Visit `/AudioNormalization` to edit policies at any scope, observe the
  per-library consistency-band breakdown, and resolve held-for-review files.
- Run `py Scripts/SweepAudioPolicyForExistingFiles.py` to dry-run a sweep
  of the library; pass `--apply` to mark `AdmissionDeferReason` on the
  reported rows.
- New TranscodeQueue rows get the `AudioPolicyJson` snapshot via the
  "Run policy snapshot" button on the Dashboard tab OR via
  `POST /api/AudioNormalization/SnapshotPolicies` OR via a cron of
  `BackfillRecentInserts`.
- Disable normalization at a scope by setting `Enabled=false` in the
  Settings form for that scope.

WHAT YOU NEED TO EXECUTE:
- WorkerService restart on I9 + larry-worker-1 to pick up the new
  shape audio paths + post-encode probe hook. I deferred this because
  two transcode jobs were running on I9 at delivery time -- killing
  them was not in scope. Operator command:
  `Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
   Where-Object {$_.CommandLine -like '*WorkerService*'} |
   ForEach-Object {Stop-Process -Id $_.ProcessId -Force}` and then
  re-launch via StartMediaVortex.py.
- Library sweep apply decision: dry-run flagged 22
  `invalid_loudness_measurement` + 1918 `ungainable_all_streams`
  (4.1% of library, above the 1% escalation threshold). Recommend
  (a) loosen LoudnessTolerance to 6.0 via the Settings page, or
  (b) flip global UngainablePolicy from 'adaptive' to 'limiter', then
  re-run the sweep. The 22 invalid rows are safe to apply unconditionally.
- Live ffprobe verification on a real encoded output (Stage 7b DialNorm
  + Stage 8 AchievedLufs in `TranscodeAttempts.AudioTracksEmittedJson`) --
  available after the WorkerService restart picks up a Quick / Transcode /
  Remux job.

CRITERIA VERIFICATION: see `## Verification` block above. 19/23 fully
delivered; 4/23 partial with documented deferral paths.

DECISIONS I MADE:
- Kept `Features/AudioCompletion/AudioCompletionService` alive because
  Compliance + FileReplacement + MediaProbe pipelines consume the
  `AudioComplete` + `AudioCorruptSuspect` flag via that service. The
  shape-side absorption (stream-copy decision in emitter) is complete;
  the audio-state-machine for compliance is out of scope. This makes C22
  PARTIAL rather than FULL.
- Did not hook the admission gate into QueueManagementBusinessService's
  bulk-INSERT paths. QMBS is 2552 LOC with 8 colocated feature docs; the
  R1 hook refused 4 attempts to edit it. Per
  `feedback_extraction_on_friction.md` decomposed: gate's
  `BackfillRecentInserts` SQL is the snapshot mechanism, invoked via the
  GUI button / API endpoint / cron. Auto-snapshot via QMBS edit deferred
  until QMBS is decomposed (separate directive).
- Default `_StubLanguageIdBackend` for `LanguageEnrichmentService`
  returns `{Language: 'und'}`. Real Whisper-class model integration is
  follow-up work -- the seam is wired, the model is the deliverable for a
  future directive. C19 is therefore PARTIAL.
- Deleted `Scripts/SQLScripts/BackfillLoudnessThreshold.py` instead of
  refactoring its R4 env-var bootstrap. The script's purpose
  (re-measure files missing the integrated threshold) is absorbed by
  `AudioRemeasurementService`; refactoring it would have charged R4 rent
  on unrelated debt.
- Deleted `Tests/Contract/TestLinearLoudnormEnforcement.py`. Its
  tree-wide audit looked for the legacy `loudnorm=I=-23:LRA=7:TP=-2`
  literal chain; the new emitter constructs the filter dynamically per
  TrackStrategy so the audit is obsolete.

KNOWN GAPS / DEFERRED:
- C12 auto-snapshot in QMBS (deferred to QMBS decomposition directive).
- C19 Whisper-class model (deferred to follow-up).
- C22 AudioCompletion flag-management absorption (deferred / may stay
  separate).
- Live ffprobe smoke (deferred to next WorkerService restart).
- 1918 ungainable files awaiting operator decision on tolerance.
...
| Source artifact in directive | Target |
|---|---|
| ## Outcome 2-knob normalization narrative | `Features/AudioNormalization/audio-normalization.feature.md` ## What It Does |
| C1-C23 acceptance criteria | `audio-normalization.feature.md` ## Success Criteria |
| Resolver / Classifier / Emitter / Gate seam table (## Seams in directive) | `audio-normalization.feature.md` ## Seams (S1-S8) |
| Operator workflows (W1-W7 implicit in narrative) | `audio-normalization.feature.md` ## Workflows |
| Engineering Calls Already Made (defaults) | `audio-normalization.feature.md` -- inline in ## What It Does and seeded global row in `Scripts/SQLScripts/Create_AudioNormalizationConfig.py` |
| `linear-loudnorm.feature.md` core principles (linear-or-refused, measurement gate, never silent fallback) | `audio-normalization.feature.md` C13 / C14 + classifier REVIEW route; `Features/LoudnessAnalysis/` to be deleted (docs supplanted) |
...
Stages 8, 9, 10, 2b all landed after the initial pass.

Delivered (19 of 23): C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C13, C14,
C15, C16, C17, C18, C20, C21, C23.

Partial (4 of 23):
- C12 — gate + Backfill SQL + manual POST /api/AudioNormalization/SnapshotPolicies
  endpoint. Auto-snapshot hook into QMBS bulk-INSERT deferred (file decomposition
  per feedback_extraction_on_friction.md).
- C19 — service scaffolded with stub backend; Whisper-class model integration
  is the follow-up.
- C22 — shape-side absorbed (RemuxShape / TranscodeShape / SubtitleFixShape
  no longer reference AudioCompletionService). Compliance / FileReplacement /
  MediaProbe still read MediaFiles.AudioComplete + AudioCorruptSuspect via
  AudioCompletionService -- that audio-state machine is out of this directive.

Not delivered (0 of 23). Live-encode ffprobe smokes pending next WorkerService
restart (drain not safe with active transcodes in-flight at session end).

116 contract tests green: 60 (foundations) + 17 (shapes) + 4 (post-encode) +
5 (enrichment) + 30 (gate / classifier / handler / remeasure / etc).
Production code paths verified through tests.
