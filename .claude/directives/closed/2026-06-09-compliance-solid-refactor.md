...
**DELIVERING** -- C15 + C22 satisfied at literal-zero grep level after Phase 7 (commit 52b1861): legacy MediaFiles columns (`recommendedmode`, `needstranscode`, `needsquick`) dropped from the live DB along with `*_legacy` rename canaries; dependent indexes (`idx_mediafiles_needs_*_legacy`, `idx_mediafiles_next_transcode_batch`) cascade-removed; 11 callers migrated to `WorkBucket` / `OperationsNeededCsv` / `ComplianceGateBlocked` (Features + Scripts + Tests/Pipeline); 7 legacy migration scripts removed from the tree; new `Tests/Pipeline/Harness/HarnessPathResolver.py` extracted (SRP); 3 operator scripts relocated `Scripts/SQLScripts/` -> `Scripts/`; new `Scripts/SQLScripts/SeedTestHarnessFallbacks.py` seeds `SystemSettings.DefaultTestWorkerName` + `DefaultTestFFmpegPath` (R4 env-var replacement) and dedups + adds `UNIQUE (SettingKey)`. 51/51 contract tests pass post-drop; 8 smoke endpoints (`Activity/LibraryCompliance`, `ShowSettings/NextTranscodeBatch`+`SmartPopulate`, `Compliance/{Buckets,Preview,TranscodeRules,...}`) respond with new shape and live counts. Awaiting operator authorization to move directive to `.claude/directives/closed/2026-06-09-compliance-solid-refactor.md` and pop `.claude/current-feature` stack.
...
| Drop scripts authored, revised to target `*_legacy`, executed live, then DELETED from tree (ephemeral migration; effect captured in live DB) | `Scripts/SQLScripts/{DropRecommendedModeColumn,DropNeedsFlags}.py` -- gone | Done (52b1861) |
| Rename canary + rollback authored, ran in Phase 1, then DELETED from tree | `Scripts/SQLScripts/{RenameLegacyComplianceColumns,RollbackRenameLegacyComplianceColumns}.py` -- gone | Done (52b1861) |
| Phase 7 cleanup: extracted harness path resolution (SRP) | `Tests/Pipeline/Harness/HarnessPathResolver.py` (NEW) -- BuildHarnessWorker + ResolveLocalPath + ResolveLocalPathForMediaFile | Done (52b1861) |
| Phase 7 cleanup: SystemSettings-backed test-harness fallbacks (R4 env-var replacement) + dedup + UNIQUE(SettingKey) | `Scripts/SQLScripts/SeedTestHarnessFallbacks.py` (NEW) | Done (52b1861) |
| Phase 7 cleanup: operator scripts relocated out of `Scripts/SQLScripts/` (not migrations) | `Scripts/{AuditOrigRecovery,RecoverOrigSurvivors,QueueRemux}.py` (moved) | Done (52b1861) |
| Phase 7 cleanup: legacy add-migration scripts deleted (effect captured in live schema) | `Scripts/SQLScripts/{AddNeedsQuickAndNeedsTranscodeFlags,AddNextTranscodeBatchIndex,AddRecommendedModeColumn}.py` -- gone | Done (52b1861) |
...
| C15 (RecommendedMode retired at DELIVERING) | Live drop executed against `recommendedmode_legacy` (rename canary from Phase 1); `\d MediaFiles` shows no `recommendedmode` (or `_legacy`) column; `grep -rn 'RecommendedMode' --include='*.py' .` exit=1 (zero matches); drop script idempotent on re-run. | 52b1861 |
...
| C22 (NeedsTranscode/NeedsQuick retired) | Live drop executed against `needsquick_legacy` + `needstranscode_legacy`; `\d MediaFiles` shows no `needsquick`/`needstranscode` (or `_legacy`) columns; `idx_mediafiles_needs_*_legacy` + `idx_mediafiles_next_transcode_batch` cascade-dropped; `grep -rn 'NeedsTranscode\|NeedsQuick' --include='*.py' .` exit=1 (zero matches); drop script idempotent on re-run. | 52b1861 |
