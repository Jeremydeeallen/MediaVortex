...
- Jellyfin push-notify reads its config fresh from `SystemSettings` on every call. Reuses the existing `JellyfinHost`/`JellyfinApiPort`/`JellyfinApiKey` rows (shared with `Features/Optimization`). No dry-run gate -- if media changes on disk, Jellyfin is notified. See `jellyfin-push-notify.feature.md`.
