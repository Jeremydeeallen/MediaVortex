...
| ST3 | ActiveJobs orphans (QualityTestingService) | `DELETE FROM ActiveJobs WHERE ServiceName='QualityTestingService' AND QueueId NOT IN (SELECT Id FROM QualityTestingQueue)` | Same WARN-per-removal pattern as ST2. |
