...
| S3 | `ST4` start/stop | this flow | Service Start/Stop methods on `ProcessTranscodeQueueService`, `ProcessQualityTestQueueService`, `ProcessRemuxQueueService`, `ContinuousScanService` | Each service runs a claim loop only when started; Stop joins on in-flight | `transcode.flow.md::S1` and `remux.flow.md::S1` claim queries only execute when corresponding capability is on |
