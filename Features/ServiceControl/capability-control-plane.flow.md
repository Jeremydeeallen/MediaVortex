...
| ST2 | Read flags fresh | `_LoadCapabilitiesFromDB()` | `SELECT TranscodeEnabled, QualityTestEnabled, RemuxEnabled, ScanEnabled, Status, MaxConcurrentTranscodeJobs, MaxConcurrentQualityTestJobs, MaxConcurrentRemuxJobs FROM Workers WHERE WorkerName=%s`. No `self._cached_*` (R3). |
...
| ST4 | Apply | `_ApplyCapabilities()` + `_ApplyConcurrencyChanges()` | Start/Stop per-capability services on change; rebind thread-pool size on running services when the `MaxConcurrent*Jobs` column changes. |
...
| S3 | `ST4` start/stop | this flow | Service Start/Stop methods on `ProcessTranscodeQueueService`, `ProcessQualityTestQueueService`, `ProcessRemuxQueueService`, `ContinuousScanService` | Each service runs a claim loop only when started; Stop joins on in-flight | `transcode.flow.md::S1` claim query (all modes; Remux-mode shape documented in `transcode.flow.md ST6 Strategy variants`) only executes when corresponding capability is on |
| S4 | `ST4` concurrency rebind | this flow | Live thread-pool size on each running service | Queue loop checks `len(ActiveJobs) < MaxConcurrentJobs` per iteration; new value applies on next claim | UPDATE `MaxConcurrentTranscodeJobs` from 1 to 2; observe second concurrent claim within one interval |
