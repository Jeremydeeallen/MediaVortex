...
6. Each card's "Add Batch" button submits only its own items to `/api/WorkBucket/AddSuggestionsToQueue`. The TV add does not pick up Movies items, and vice versa. Verifiable: select 5 TV items, click Add Batch on TV card, observe 5 queue rows appear with paths starting `T:\`; no `M:\` rows added.

7. Sticky batch size for the Movies card is persisted under localStorage key `WorkBucket.MoviesBatchSize` (parallel to the existing `WorkBucket.BatchSize` for TV). Default 100, max 1000. Verifiable: change Movies size to 250, reload page, observe Movies card still shows 250 selected; TV card unchanged.
...
COMPLETE 2026-05-30. WebService restarted; both cards render and populate independently (verified via /Work/Transcode page HTML inspection + endpoint smoke tests: TV returned 11,645 candidates, Movies returned 1,063).
...
- [x] 5. Wired search-debounce + size-change handlers for the Movies card (parallel to TV). Sticky size under `WorkBucket.MoviesBatchSize` localStorage key.
...
```
Templates/WorkBucket.html                                  -- two cards, parallel JS
