...
| ST6 | Worker claims | `ClaimNextPendingTranscodeJob` (`transcode.flow.md::S1`) | Standard `ORDER BY Priority DESC, DateAdded ASC` surfaces the boosted rows first. |
...
| S3 | `ST5` cascade integration | `QueueManagementBusinessService._EvaluateCompliance` + insert paths | New AudioFix queue rows for the pinned folder land with `Priority = base + BoostBy` | Workers claim boosted rows first per `transcode.flow.md::S1` ORDER BY | Add a new file in a pinned folder, populate queue, observe inserted row's Priority reflects boost |
