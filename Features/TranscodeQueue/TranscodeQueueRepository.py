        ...
        try:
            ...
            try:
                ...
                query = (
                    ...
                    "    ClaimedAt = NOW(), "
                    "    DateStarted = NOW(), "
                    "    Filename = COALESCE((SELECT FileName FROM MediaFiles WHERE Id = tq_outer.MediaFileId), tq_outer.Filename), "
                    "    RelativePath = COALESCE((SELECT RelativePath FROM MediaFiles WHERE Id = tq_outer.MediaFileId), tq_outer.RelativePath), "
                    "    StorageRootId = COALESCE((SELECT StorageRootId FROM MediaFiles WHERE Id = tq_outer.MediaFileId), tq_outer.StorageRootId), "
                    "    SizeMB = COALESCE((SELECT SizeMB FROM MediaFiles WHERE Id = tq_outer.MediaFileId), tq_outer.SizeMB), "
                    "    SizeBytes = COALESCE(((SELECT SizeMB FROM MediaFiles WHERE Id = tq_outer.MediaFileId) * 1024.0 * 1024.0)::bigint, tq_outer.SizeBytes) "
                    "WHERE tq_outer.Id = ( "
                    "    SELECT tq.Id FROM TranscodeQueue tq "
                    "    INNER JOIN ProcessingModes pm ON pm.Name = tq.ProcessingMode "
                    ...
                    "          ) "
                    "          AND (NOT pm.RequiresProfileGates "
                    "               OR COALESCE(p.codec,'') <> 'av1' "
                ...
                cursor.execute(
                    query,
                    (WorkerName, WorkerName, AcceptsInterlaced) + NvencParams + QsvParams + InflightParams,
                )
                row = cursor.fetchone()
                connection.commit()
