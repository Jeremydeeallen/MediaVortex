...
12. **Enqueue non-null contract (S3).** Every production producer that INSERTs a `TranscodeQueue` row satisfies an identical non-null column set: `StorageRootId`, `RelativePath`, `ProcessingMode`, and `AudioPolicyJson` (backfilled synchronously by an `AudioPolicyAdmissionGate` snapshot call within the producing method). Top-level producers: `QueueManagementBusinessService.AddJobToQueue` (Web GUI / WorkBucket / requeue), `QueueManagementBusinessService.AddSuggestionsToQueue` + `.QueueAllMatching` (bulk), `Features/QualityTesting/QualityTestController` variant-set admission. Repository-layer `TranscodeQueueRepository.SaveTranscodeQueueItem` + `.BulkInsertQueueItems` are internal helpers driven by those producers -- the snapshot is invoked by the caller, not the repo. Verifiable: `py -m pytest Tests/Contract/TestEnqueueContract.py` passes (source-inspection asserts every top-level producer calls a snapshot hook + every INSERT stanza names `StorageRootId`, `RelativePath`, `ProcessingMode`; live-DB audit asserts recent Pending rows all have the four columns non-null); SQL `SELECT COUNT(DISTINCT (audiopolicyjson IS NOT NULL, storagerootid IS NOT NULL, relativepath IS NOT NULL, processingmode IS NOT NULL)) FROM transcodequeue WHERE dateadded > <cutover>` returns 1.


13. **[BUG-0092]** Contract tests that claim TranscodeQueue rows (via hardcoded test-worker names like _test-*) must either DELETE the row or release it back to Status=Pending in tearDown. And/or the abandonment sweeper widens to catch TranscodeQueue rows whose ClaimedBy has no matching Workers row. Verifiable: SELECT COUNT(*) FROM TranscodeQueue WHERE ClaimedBy LIKE _test-% returns 0 after any pytest run plus a fresh sweep cycle.
...
- **Admission INSERTs** in `TranscodeQueueRepository.SaveTranscodeQueueItem` + `.BulkInsertQueueItems`, `QueueManagementBusinessService.AddSuggestionsToQueue` + `.QueueAllMatching` all carry `ON CONFLICT (MediaFileId) WHERE Status='Pending' AND TestVariantSetId IS NULL DO NOTHING RETURNING Id`. The PostgreSQL engine serializes concurrent INSERTs at the unique-index level; the winner gets the new Id back from `RETURNING`, every loser returns `None`/zero rowcount silently — no exception path involved.
