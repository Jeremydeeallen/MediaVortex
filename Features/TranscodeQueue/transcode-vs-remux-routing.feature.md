...
26. **[BUG-0003 SUPERSEDED by audio-dialog-boost-real (2026-07-02)]** Under the two-track Source of Truth every encoded output now ships Track 0 (Original) + Track 1 (Dialog Boost), regardless of source `AudioComplete` state. Byte-identical Remux is no longer the compliant outcome -- Track 0 is emitted with linear-mode loudnorm per `Features/AudioNormalization/audio-normalization.feature.md` C36 + C37 (single audio emit path across all six ProcessingModes). The `AudioComplete` flag is now a compliance / self-healing signal only, not a routing signal.
