...
26. Under the two-track Source of Truth every encoded output ships Track 0 (Original) + Track 1 (Dialog Boost), regardless of source `AudioComplete` state. Track 0 is emitted with linear-mode loudnorm per `Features/AudioNormalization/audio-normalization.feature.md` C36 + C37 (single audio emit path across all six ProcessingModes). The `AudioComplete` flag is a compliance / self-healing signal only, not a routing signal.
