...
| S3 | `ST6 -> downstream` (classifier write -> queue admission) | `ContentClassifierRepository.WriteAssignment` | `MediaFiles.(AssignedProfile TEXT, AssignedProfileSource TEXT)` -- single UPDATE statement | `QueueManagementBusinessService.AddJobToQueue` consumes `AssignedProfile` at enqueue time (profile lookup for encoder knobs); compliance verticals do NOT read this column (C33) | `SELECT AssignedProfileSource, COUNT(*) FROM MediaFiles GROUP BY AssignedProfileSource` -- 'classifier' bucket non-zero after first probe pass |
