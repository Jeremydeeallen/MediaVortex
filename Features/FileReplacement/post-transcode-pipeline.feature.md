...
- [x] Add TFP DELETE to `PostTranscodeDispositionService._CommitDisposition` for terminal dispositions `Reject`/`NoReplace`/`Requeue`/`Discard` (criterion 15). `Replace` defers to FileReplacement's existing success-branch cleanup since the canonical paths are still needed.
