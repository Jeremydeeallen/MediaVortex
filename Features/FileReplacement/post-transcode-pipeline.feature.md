...
1. ~~When QualityTestRequired=False on the TranscodeAttempt, ShouldQualityTestService skips the quality test queue entirely and proceeds directly to file cleanup. No QualityTestQueue row is created.~~ Superseded -- the disposition function returns `(Replace, QualityTestNotRequired)` for this case.
...
3. ~~When quality testing capability is not running and QualityTestRequired=True, the quality test is still queued.~~ Superseded -- attempt lands `Pending/AwaitingVmaf` regardless of worker-online state (`qt-queue-visibility-and-override.C1`).
...
15. TemporaryFilePaths rows are deleted at *every* TranscodeAttempt terminal state, not just successful Replace. A row exists iff the attempt is still in flight (`TranscodeAttempts.Success IS NULL AND CompletedDate IS NULL`). Terminal states that must trigger TFP deletion: FFmpeg failure, FFprobe-verify failure, VMAF failure, every Disposition value (`Replace`, `Reject`, `Discard`, `NoReplace`, `Requeue`), and crash-recovery transitions. Verifiable: with all workers Paused, `SELECT COUNT(*) FROM TemporaryFilePaths tfp JOIN TranscodeAttempts ta ON ta.Id = tfp.TranscodeAttemptId WHERE ta.Success IS NOT NULL` returns 0. **Implementation:** the chokepoint is `AttemptCleanupService.Cleanup(TranscodeAttemptId)` invoked from `DispositionDispatcher._MaybeCleanupTfp` for terminal dispositions (`Reject`, `NoReplace`, `Requeue`, `Discard`); `FileReplacementBusinessService.ProcessFileReplacement` calls it on `Replace` success (BUG-0010 closure). `HandleJobFailure` in `ProcessTranscodeQueueService` cleans TFP for FFmpeg/FFprobe-verify failures.
...
- [x] Add TFP DELETE to `DispositionDispatcher._MaybeCleanupArtifacts` for terminal dispositions `Reject`/`Requeue` (criterion 15) unless `RetainInprogressPolicy.ShouldRetain(Reason)` (TestMode). `Replace` defers to FileReplacement's existing success-branch cleanup since the canonical paths are still needed.
