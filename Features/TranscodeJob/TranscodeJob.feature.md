...
- [BUG-0061] **TranscodeAttempts INSERT MUST set MediaFileId.** As of 2026-06-12 the DB contains 1455 `TranscodeAttempts` rows with `MediaFileId IS NULL` going back to 2025-10-15 -- all `Success=FALSE` (the orphan-failure population from BUG-0055 triage). Without `MediaFileId` the row cannot be joined back to the source file for operator diagnosis, the per-MediaFile failure-cap predicate from BUG-0055 cannot count them, and they pollute every aggregate that filters `WHERE Success=FALSE`. Fixed means: (a) every `TranscodeAttempts` INSERT path (success AND failure, every ProcessingMode -- Transcode / Remux / Quick / AudioFix / SubtitleFix / TestVariant) sets `MediaFileId` from the resolved Job row or refuses the INSERT with a logged exception; the column gains a `NOT NULL` constraint enforced by migration after the cleanup. (b) A one-shot script `Scripts/SQLScripts/CleanupOrphanFailedAttempts.py` triages the existing 1455 rows: dedupe by `ErrorMessage + AttemptDate-rounded-to-minute + WorkerName` (most are likely same-batch noise), best-effort backfill `MediaFileId` from `TranscodeQueue.Id` <-> `TranscodeAttempts.Id` correlation where the queue row was caught before deletion, archive the rest to a timestamped `Reports/OrphanFailedAttempts-YYYY-MM-DD.csv` and delete from `TranscodeAttempts`. (c) Root-cause audit identifies which code path INSERTs with `MediaFileId=NULL`; `HandleJobFailure` at `Features/TranscodeJob/ProcessTranscodeQueueService.py:1100` is the prime suspect (it builds `TranscodeAttemptModel` without `MediaFileId` if the existing TranscodeAttemptId is missing -- line 1113-1133). Verifiable: post-fix `SELECT COUNT(*) FROM TranscodeAttempts WHERE MediaFileId IS NULL` returns 0; post-fix `\d TranscodeAttempts` shows `MediaFileId BIGINT NOT NULL`; insert a forced-failure transcode and confirm the resulting row carries the correct MediaFileId.
...
2. **Burn-in subtitle codec list triplication.** Three slightly-different definitions of "which subtitle codecs need fixing":
   ...
   - `ActivityRepository.py:130-132` -- `ass|ssa|hdmv_pgs|pgssub|dvd_subtitle|dvdsub` (broad)
   - `OptimizationViewModel.py:580` -- lowercase substring match

   Centralize on one definition (likely the broad ActivityRepository set) in a `SubtitleCompliance` domain helper. Deferred to the same future directive.
