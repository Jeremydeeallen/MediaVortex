...
C4. **`Features/TranscodeJob/Emit/Slots/SubtitleSlot.py` codec-aware filter.** MP4 target maps only subtitle streams whose ffprobe `codec_name` is in `TEXT_SUB_CODECS` (subrip / srt / ass / ssa / mov_text / tx3g / webvtt / vtt / microdvd / text). Drops `IMAGE_SUB_CODECS` (PGS / DVDSUB / DVBSUB / xsub -- need OCR, out of scope), `UNDECODABLE_SUB_CODECS` = {'unknown', 'none', 'null', ''} (ffmpeg has no decoder), and any other codec name not in either set. When `SubtitleStreams=[(idx,codec),...]` is present, emits explicit `-map 0:<idx>?` per survivor + single `-c:s mov_text`. Fallback (no SubtitleStreams + all Formats in TEXT_SUB_CODECS): `-map 0:s? -c:s mov_text` shortcut. Otherwise drop-all with WARN. MKV target: `-map 0:s? -c:s copy` (container accepts arbitrary codecs). Every drop logs one INFO line naming counts + `(index, codec)` detail. Verifiable: `Tests/Contract/TestSubtitleSlotCodecFilter.py` + `Tests/Contract/TestCommandComposer.py::test_subtitle_slot_always_fires_mov_text_on_mp4`.
