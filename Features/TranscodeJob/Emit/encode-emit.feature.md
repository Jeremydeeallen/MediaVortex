...
C9. **Plan tuple derived by PlanFactory from per-dimension compliance flags** (transcode.flow.md D2). `Features/TranscodeJob/Emit/Plan.py` defines `@dataclass(frozen=True) class Plan(VideoOp, AudioOp, SubtitleOp, ContainerOp)`. `PlanFactory.FromComplianceState(MediaFile)` -> `(VideoOp=Copy if VideoCompliant else Reencode, AudioOp=Copy if AudioCompliant else Reencode, SubtitleOp=Preserve, ContainerOp=Preserve if ContainerCompliant else Mp4)`. Any None compliance flag raises `ValueError` (admission gate should have deferred). Verifiable: `Tests/Contract/TestPlanFactoryFromComplianceState.py` (8 flag combos + 3 None guards).

C10. **ContainerSlot emits `-f mp4 -movflags +faststart+use_metadata_tags` for both `Mp4` and `Preserve` targets** (D5 output-mp4-always: `Preserve` means source container was already compliant; ffmpeg args are identical, only the semantic label differs). Closes the BUG-0048 class: every argv contains both flags regardless of Op. Verifiable: `Tests/Contract/TestCommandComposer.py::TestContainerSlot::test_mp4_emits_f_mp4_and_faststart` + `::test_preserve_emits_same_as_mp4`.
...
| S2 | `CommandComposer.Build -> PlanFactory.FromComplianceState` | CommandComposer | `MediaFile.VideoCompliant / AudioCompliant / ContainerCompliant: bool` | `Plan(VideoOp, AudioOp, SubtitleOp, ContainerOp)`; None flag raises `ValueError` | `Tests/Contract/TestPlanFactoryFromComplianceState.py` |
...
ACTIVE. Single `CommandComposer.Build(MediaFile, Job, Context)` owns every ffmpeg argv construction. Four Slot services (`VideoSlot`, `AudioSlot`, `SubtitleSlot`, `ContainerSlot`) compose in fixed order keyed on `Plan{VideoOp, AudioOp, SubtitleOp, ContainerOp}` derived from `(VideoCompliant, AudioCompliant, ContainerCompliant)` via `PlanFactory.FromComplianceState(MediaFile)`. Compliance-driven Plan landed under `plan-factory-driven-by-compliance-flags` 2026-08-08 (Phase 4).
