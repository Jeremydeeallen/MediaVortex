...
| S7 | `PreEncode substep -> ST3` (state-store) | `PreEncodeAudioPipeline.Run` invokes `ProgressReporter(Phase, Percent, Info)` -> `SaveTranscodeProgress(AttemptId, Phase, Percent, ...)` per substep boundary + per Demucs tqdm tick | `TranscodeProgress` row `(TranscodeAttemptId, CurrentPhase, ProgressPercent, LastProgressUpdate)` | `_BuildActiveJobs` LATERAL-JOIN reads the latest row per attempt; renderer maps Phase name -> cell + Progress % -> cell | Live smoke on `/Activity` during pre-encode |
| S8 | `TranscodeAttempts.AttemptDate -> Elapsed cell` (state-store) | Repository projects `AttemptDate` per row | timestamptz | Snapshot computes `NOW() - AttemptDate` via `FormatDuration`; renderer prints the string verbatim (no client-side ticker) | Live smoke: Elapsed increments ~5 s per snapshot poll |
