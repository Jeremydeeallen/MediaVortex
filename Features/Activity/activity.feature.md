...
C9. **JS mutation handlers do not hard-couple to render-fn names.** No `.then(<RenderFnName>)` / `.then(<LoadX>)` / `.then(<RefreshX>)` callback appears on any operator UI template. Handlers do their mutation; the render layer (SSE snapshot stream + 5s poll fallback) owns refresh. If sub-second visual response is required, the handler dispatches a domain event and the render layer subscribes. Verifiable: `Tests/Contract/TestOperatorUIHandlersDecoupled.py` asserts `grep -rnP "\.then\(Render|\.then\(Load[A-Z]|\.then\(Refresh" Templates/*.html` returns 0. Reason: BUG-0094 (2026-08-26) -- Reset button on hung-banner threw `ReferenceError: LoadActivity is not defined` because the render entry was renamed but the mutation handler's hard reference was not swept. Design fix (delete the callback) removes the class of failure.
...
| S2 | Snapshot endpoint contract | `DashboardSnapshotService.BuildSnapshot` -> `/api/Activity/Snapshot` | `{ActiveJobs, ActiveScans, QueueCounts, BadgeState, StaleProgressThresholdSec}` | `Templates/Activity.html` `RenderSnapshot` (invoked by `PollOnce` + `EventSource` `onmessage`) updates both tables | `activity-dashboard.flow.md` S1 |
