...
19. **[BUG-0042] The Active Jobs list view on `/Activity` renders every in-flight queue row that the header badge counts -- VMAF runs (`QualityTestingQueue.Status='Running'`) must appear alongside transcode runs.** Today the badge sums transcode + VMAF in-flight rows but the visible list omits VMAF, so an operator with VMAF-only work in flight sees a non-zero count and an empty (or partial) list, infers the workers are hung, and kills/restarts them -- orphaning the claimed VMAF rows in `Running` state and requiring manual SQL cleanup. Fixed means: badge count and list cardinality are sourced from the same backend query (or two queries with proven equal cardinality), VMAF rows render with worker name + claim age + a VMAF-progress indicator (per `Features/QualityTesting/*` progress emit), and a synthetic test with 0 transcodes + N>=1 VMAF jobs running shows exactly N rows in the list. Verifiable: pause all transcode work, start one VMAF job, confirm badge=1 AND list shows 1 row identifying the VMAF run and its claiming worker. **MET 2026-06-03**: `Features/QualityTesting/QualityTestRepository.GetRunningQualityTestProgress` drives from `ActiveJobs WHERE ServiceName='QualityTestService'` LEFT JOIN `QualityTestProgress`/`QualityTestingQueue`/`TranscodeAttempts`. Live canary against 12 orphan QualityTestService claims: `/api/QualityTesting/Progress` returned `Jobs.Count=12` matching `/api/SQLQueries/GetActiveJobs` `QualityTestService=12`; each row carries `WorkerName`, `ClaimedAt`, `ClaimAgeSec`; orphan rows carry `ProgressPercentage=NULL` and render with a stale-claim badge per `Templates/Activity.html` `RenderActiveJobs`.

20. **[BUG-0063] FPS and Speed columns are smoothed and treat stale samples honestly.** Today `TranscodeProgressModel.CurrentFPS` and `CurrentSpeed` are raw spot values from the most recent FFmpeg progress line and render verbatim in the Active Jobs table; FFmpeg's per-second emit jumps between e.g. 100 / 5 / 95 / 0 / 80 during keyframe intervals and the UI fluctuates wildly or appears frozen at 0. Fixed means: (a) the rendered FPS column shows a rolling 10-sample (or 30-second window, whichever is smaller) arithmetic mean of `CurrentFPS` samples for the same `TranscodeAttemptId`, not the raw last value; (b) the rendered Speed column applies the same smoothing; (c) when no progress sample has arrived in the last `StaleProgressThresholdSec` (new SystemSettings row, default 15s), the FPS/Speed cells render as `--` rather than the last value (so the operator can tell "actually paused" from "just hasn't tickled the row yet"); (d) the smoothing window is reset to the new value when a worker claim transitions to a new TranscodeAttemptId, so a fresh job doesn't inherit the previous job's average. Verifiable: synthetic injection of progress samples `[100, 5, 95, 0, 80, 105, 8, 90, 0, 100]` renders as `58.3` (mean), not the trailing `100`; subsequent 20-second silence renders as `--`, not the stale `100`.

21. **[BUG-0063] ETA counts down second-by-second; recomputes only on material change.** Today the ETA cell on the Active Jobs table is recomputed from scratch each render poll using the latest CurrentFPS spot value, so it ticks erratically -- jumps from `00:14:23` to `00:08:11` to `00:17:55` purely from FFmpeg's per-second FPS noise. Fixed means: (a) the client maintains a per-job ETA timer that decrements 1 second per second between server polls (smooth countdown); (b) on each poll, the server-computed ETA is compared to the client's running ETA -- if the absolute delta is <=5 seconds, the client's timer wins (no recompute); if >5 seconds, the client resets to the server value (material change, recompute justified); (c) the server-computed ETA uses the same smoothed FPS from C20, not the raw spot value, so the polled value itself is stable; (d) ETA renders as `--:--:--` when smoothed FPS is unavailable or stale (per C20's stale-sample rule). Verifiable: open Active Jobs on a long encode, observe ETA decrements 1s per second between 5s polls; force a step-change in encode rate (e.g. by toggling worker concurrency), observe the ETA snaps to the new estimate on the next poll.

22. **[BUG-0063] Active Jobs panel keeps rendering jobs claimed by Draining workers.** Today when an operator flips a worker to `Status='Draining'`, the worker continues running its in-flight job until it completes -- but the Active Jobs panel removes that job's row from view while it is still running, leaving the operator blind to its progress and unable to see ETA / FPS during drain. Likely cause: the panel's source query joins `Workers` with an implicit `Status='Online'` filter, or the frontend filters rendered rows by worker status. Fixed means: the Active Jobs list includes every job whose `TranscodeAttempts.Success IS NULL` AND has a live `ActiveJobs` row, regardless of the owning worker's `Workers.Status`. Worker tile and Active Jobs panel are decoupled: a draining worker shows the `Draining` badge on its tile AND its still-running job appears in the Active Jobs list with the worker's name. Verifiable: start a long encode on `larry-worker-1`, flip larry to Draining mid-encode, confirm the Active Jobs row for that encode remains visible with continuing progress until the encode completes naturally; confirm the worker tile badge shows `Draining`.
