...
|---|---|---|---|---|
| UI → PATCH /api/profiles/<id>/knobs | `Settings.html` `ShowProfileKnobs` JS (`ReadField` coercions) | JSON `{Profile: {col: val}, Thresholds: [{Id, col: val}]}`. `bool_int` fields (UseNvidiaHardware): `parseInt` → int 0/1; `bool` fields (FastStart, QualityTestRequired): JS boolean; `int`: `parseInt`; `float`: `parseFloat` | `ProfileController.patch_profile_knobs` filters keys through `PROFILE_COLS`/`THRESHOLD_COLS` allowlist; unknown keys silently dropped | PATCH all 21 Profile + 20 Threshold fields; verify non-allowlisted fields absent in DB. `QualityTestRequired=FALSE` round-trip: PATCH false; next `TranscodeAttempt` for that profile gets `QualityTestRequired=FALSE` and skips VMAF per `transcode.flow.md ST7`. |
| PATCH → PostgreSQL UPDATE | `ProfileController.patch_profile_knobs` -- column names from allowlist, parameterized values | `UPDATE Profiles SET col=%s WHERE Id=%s` / `UPDATE ProfileThresholds SET col=%s WHERE Id=%s AND ProfileId=%s` | PostgreSQL `Profiles` (`usenvidiahardware BIGINT`, `preset BIGINT`, `faststart BOOLEAN`) + `ProfileThresholds` (`maxbitratemultiplier NUMERIC`) | Post-PATCH `SELECT` confirms values round-trip correctly (esp. bool_int → bigint) |
