...
C7. `WorkBucket` is a GENERATED column with a MediaVortex-terminal short-circuit scoped to Dialog-Boost-carrying outputs, ahead of the compliance-flag branches (see `transcode.flow.md` D7): `WHEN TranscodedByMediaVortex = TRUE AND HasDialogBoostTrack = TRUE THEN 'Compliant' / WHEN videocompliant IS NULL OR containercompliant IS NULL OR audiocompliant IS NULL THEN 'Unclassified' / WHEN videocompliant AND containercompliant AND audiocompliant THEN 'Compliant' / WHEN NOT videocompliant THEN 'Transcode' / WHEN NOT containercompliant THEN 'Remux' / ELSE 'AudioFix'`. Every MediaFile row has a non-NULL bucket. MediaVortex outputs that carry the Dialog Boost track are terminal (re-encoding compounds Demucs artifacts + can't start from a deleted source); MediaVortex outputs WITHOUT Dialog Boost fall through the compliance branches so operator can enforce the strict Dialog-Boost policy (`audio-vertical-dialog-boost-enforcement`). Verifiable: `SELECT generation_expression FROM information_schema.columns WHERE table_name='mediafiles' AND column_name='workbucket'` returns the six-branch CASE with the two-predicate first branch.

C8. Every compliance evaluator resolves policy directly, no hierarchy resolver. `AudioVertical.Evaluate` reads `AudioComplianceRules`. `VideoVertical.Evaluate` reads `VideoComplianceThresholds` (per-resolution multiplier) + `Profiles.Family` + `ProfileThresholds.TargetKbps` (Tier 1). `ContainerVertical.Evaluate` reads `ContainerComplianceRules`. Each returns `(True|False|None, str|None)` without invoking `EffectiveProfileResolver`. `AssignedProfile` is a HINT for Family lookup, not a per-file policy override. Verifiable: `grep -n "EffectiveProfileResolver" Features/VideoEncoding/VideoVertical.py Features/ContainerFormat/ContainerVertical.py Features/AudioNormalization/AudioVertical.py` returns 0 lines. Contract test `Tests/Contract/TestVerticalsAreProfileIndependent.py` asserts each vertical accepts a MediaFile with `AssignedProfile=NULL` and returns a decision without raising.
