...
| W5 | Worker probes a file (scan or reprobe) | -- | `MediaProbeBusinessService._ExecuteProbe` post-flight | `QueueManagementBusinessService.RecomputeForFiles` (dual-writes legacy + new bucket columns) |
...
|---|---|---|---|---|---|
| S1 | `MediaProbeBusinessService` -> `RecomputeForFiles` | Probe completion in `_ExecuteProbe` | `MediaFileId: int` | `RecomputeForFiles([id])` evaluates + dual-writes legacy (`IsCompliant`/`RecommendedMode`/`NeedsTranscode`/`NeedsQuick`/`AdmissionDeferReason`) + new (`WorkBucket`/`OperationsNeededCsv`/`ComplianceGateBlocked`/`ComplianceEvaluatedAt`) columns on `MediaFiles` | Post-probe `SELECT ComplianceEvaluatedAt FROM MediaFiles WHERE Id=<id>` is fresh |
| S2 | `EvaluateCandidateCompliance` -> `ComplianceEvaluator.Evaluate` | `Features/FileReplacement/ComplianceGate.Evaluate` (worker pre-rename) | `CandidateRow: dict` shaped like `MediaFiles` row | `EvaluateCandidateCompliance` returns `{IsCompliant, RecommendedMode, RefusalReason}` legacy shape via delegation | `Tests/Contract/TestComplianceEngine.TestGates` |
