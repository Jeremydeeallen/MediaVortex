...
11d. **[UPDATED 2026-07-02] VMAF filter chain aligned to shootout / EncoderShootout.py.** The prior chain scaled the reference to encoded resolution using the default (bilinear) filter, did not pin color range, and applied `format=yuv420p10le` before `fps=` / `setpts=` reset. This under-scored real content by 25-32 VMAF points vs the shootout chain (Resident production 37.06 vs shootout 69.64; Walking Dead production 47.29 vs shootout 75.34, both on the same encoded bytes). Root cause: color-range histogram mismatch when source is tv-range and encoded lands in pc-range without normalization + bicubic-quality reference scaling that misses fine detail. Fix landed 2026-07-02: extracted `QualityTestingBusinessService._BuildVmafFilterChain(SourceFps, TargetWidth, TargetHeight, XmlLogPath)` static helper called by both `BuildVMAFCommand` (post-transcode path) and `RunLocalVmafForAttempt` (Mode A pre-flight path). Chain shape now: `[0:v]fps={fps},setpts=PTS-STARTPTS,scale=W:H:flags=lanczos:in_range=auto:out_range=tv,format=yuv420p10le[dist]; [1:v]fps={fps},setpts=PTS-STARTPTS,scale=W:H:flags=lanczos:in_range=auto:out_range=tv,format=yuv420p10le[ref]; [dist][ref]libvmaf=log_fmt=xml:log_path={xml}:n_threads=4`. Verifiable: any encoded file re-scored under the new chain returns a higher (and correct) VMAF than the same file scored under the pre-2026-07-02 production chain. **Historical VMAF scores 2026-05-29 to 2026-07-02 are direction-correct but magnitude-under-scored** and should be re-evaluated against the current chain before any Requeue disposition decision that relied on them; consider a fleet re-score sweep before manually re-transcoding files that were Requeued during that window.
