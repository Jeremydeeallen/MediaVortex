...
11d. **VMAF filter chain is stage-composed by `Features/QualityTesting/Vmaf/VmafFilterChainBuilder.py` per branch.** Both `[dist]` (transcoded, `-i` position 0) and `[ref]` (original, `-i` position 1) branches run the identical chain composed from `AlignmentSpec`: `setpts=PTS-STARTPTS` → `yadif=1` (if `DeinterlaceNeeded`) → `fieldmatch,decimate` (if `DetelecineNeeded`) → `fps={TargetFps}` → `scale=in_range=auto:out_range={ColorRange}` (colorspace / range normalize) → `crop={W}:{H}:{X}:{Y}` (if `EncodedCrop` set) → `scale={W}:{H}:flags=lanczos` (resize) → `format={ChromaFormat}`. Terminal: `[dist][ref]libvmaf=model=version={ModelVersion}:log_fmt=xml:log_path={xml}:n_threads=4`. Two `scale=` filters is intentional -- range normalization and resolution resize are SRP-separate stages so `ColorSpaceService` and `VmafModelSelector` each drive their own transformation. Range pinning + lanczos reference resize + fps unification vs the pre-2026-07-02 default-bilinear-no-range chain lifted real-content scores 25-32 VMAF points (Resident production 37.06 → shootout 69.64; Walking Dead 47.29 → 75.34, same encoded bytes). Verifiable: `Tests/Contract/TestVmafFilterChainBuilder.py` + `TestVmafModelSelector.py` + `TestAlignmentSpec.py` + `Docs/Codecs/4K-AV1-Streaming-Sweep-2026-07-06.md`. **Historical VMAF scores 2026-05-29 to 2026-07-02 are direction-correct but magnitude-under-scored** and should be re-evaluated before any Requeue disposition decision that relied on them.
