...
- **VMAF measurement arm** resolved 2026-05-29. Three fixes:
  ...
  2. **Chain modernized** to match the production-decision shootout: PTS reset on both inputs, unified scale with lanczos + `in_range=auto:out_range=tv` color-range pinning (NVENC outputs limited range; mismatches collapsed scores), 10-bit precision (`yuv420p10le`), libvmaf with `log_fmt=xml` + `n_threads=4`.
