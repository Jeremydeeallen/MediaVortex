...
| S4 | `ST3 -> ST4` (PROBE -> RUN_VMAF) | `QualityTestingBusinessService.BuildVMAFCommand` invokes `_BuildVmafArgvViaComposer` -> `VmafAlignmentProbe.Probe` -> `AlignmentSpec` -> `VmafModelSelector.Select` + `VmafFilterChainBuilder.Build` -> `VmafCommandComposer.Build` | ffmpeg argv: `[-ss <t>] -i <transcoded> -i <original> -lavfi <chain> -f null -`; chain composes 9 stages per branch (setpts/deint/detelecine/fps/colorspace/crop/scale/chroma/libvmaf) into `[0:v]<chain>[dist];[1:v]<chain>[ref];[dist][ref]libvmaf=model=version=<selected>:log_fmt=xml:log_path=vmaf_output.xml:n_threads=4` | `QualityTestingBusinessService.ExecuteFFmpegWithProgress` spawns ffmpeg; `MonitorVMAFProgress` thread updates `QualityTestProgress.(CurrentFps, AverageFps, EtaSeconds, ProgressPercentage)` from stderr frame lines | `QualityTestResults.FFmpegCommand` populated pre-run for audit; process return code drives the branch; `Tests/Contract/TestVmafFilterChainBuilder.py` + `TestVmafModelSelector.py` + `TestVmafCommandComposer.py` |
...
- `BuildVMAFCommand`:
  ...
  - `_BuildVmafArgvViaComposer` delegates alignment probe + command composition to `Features/QualityTesting/Vmaf/`.
...
**Code path:**
- `VmafAlignmentProbe.Probe(source, encoded)` returns `AlignmentSpec` -- immutable VO covering 13 alignment axes (color triad + range + fps + VFR + resolution + crop + deint + detelecine + bit depth + chroma + HDR + durations). Fail-loud on unparseable primaries / fps / pix_fmt / duration parity delta > 1 frame.
- `VmafModelSelector.Select(spec)` picks `VmafModel` per rules (MaxEdgePx >= 1440 -> 4K, <= 540 -> phone, HDR -> neg, else default).
- `VmafFilterChainBuilder.Build(spec, model, xml, n_threads=4)` composes 9 stages per branch (setpts / deinterlace / detelecine / fps / colorspace / crop / scale / chroma / libvmaf) into `[0:v]<chain>[dist];[1:v]<chain>[ref];[dist][ref]libvmaf=...` graph.
- `VmafCommandComposer.Build` returns ffmpeg argv (distorted first -> `[0:v]->[dist]`; reference second -> `[1:v]->[ref]`; optional `-ss`; `-f null` tail). See `QualityTesting.feature.md` C11c.
