...
10. The reason vocabulary is closed. Allowed values: `TranscodeFailed`, `InsufficientSavings_<actual>pct_below_<threshold>pct`, `QualityTestNotRequired`, `AwaitingVmaf`, `VmafBelowMin`, `VmafPassed`, `VmafAboveMax`, `VmafServicePaused`, `VmafServicePausedBypassed`, `VmafCapabilityNotConfigured`, `QualityTestingGloballyDisabled`, `PartialSuccess_AudioSlotCopied`, `PartialSuccess_VideoSlotCopied`, `PartialRetryExhausted`. No free-text reasons. `InsufficientSavings_*` is parametric: the two integers are actual savings % and configured threshold %; matched by prefix `InsufficientSavings_`. The three `Partial*` values are written directly by `JobProcessor._TryPartialFallback` (not emitted by `DispositionDecider.Decide`) and honored via the DispositionDispatcher cached-check path (`transcode.D13` -- partial-pipeline-completion). `NoSavings` (0% threshold, legacy) retired 2026-08-10 by `pre-encode-savings-gate` -- rows with historical `NoSavings` remain in the audit table but no new writes. Verifiable: `SELECT DISTINCT DispositionReason FROM TranscodeAttempts WHERE DispositionReason IS NOT NULL AND DispositionReason NOT LIKE 'InsufficientSavings_%'` returns only values from the fixed list.

11. The "why didn't this replace?" query works: `SELECT FilePath, Disposition, DispositionReason FROM TranscodeAttempts WHERE Success=true AND FileReplaced=false AND Disposition <> 'Pending'` returns one row per stuck attempt with an enumerable reason. Verifiable: induce three stuck cases (`InsufficientSavings_*`, VmafBelowMin, VmafServicePaused), run the query, observe three rows with the expected reasons.
