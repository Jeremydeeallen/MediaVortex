...
10. The reason vocabulary is closed. Allowed values: `TranscodeFailed`, `NoSavings`, `QualityTestNotRequired`, `AwaitingVmaf`, `VmafBelowMin`, `VmafPassed`, `VmafAboveMax`, `VmafServicePaused`, `VmafServicePausedBypassed`, `VmafCapabilityNotConfigured`, `QualityTestingGloballyDisabled`, `PartialSuccess_AudioSlotCopied`, `PartialSuccess_VideoSlotCopied`, `PartialRetryExhausted`. No free-text reasons. The three `Partial*` values are written directly by `JobProcessor._TryPartialFallback` (not emitted by `DispositionDecider.Decide`) and honored via the DispositionDispatcher cached-check path (`transcode.D13` -- partial-pipeline-completion). Verifiable: `SELECT DISTINCT DispositionReason FROM TranscodeAttempts WHERE DispositionReason IS NOT NULL` returns only values from this list.
