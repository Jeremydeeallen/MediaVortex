...
21. [BUG] **The operator can register and scan multiple drives/shares from any worker.** Today RootFolders are seeded under specific drive prefixes (T:\, M:\, Z:\) and a worker can only scan drives it has `WorkerShareMappings` rows for. Adding a new drive to scan requires: (a) manually inserting RootFolders rows with the correct canonical prefix, (b) adding `WorkerShareMappings` rows for every worker that can reach the new drive, and (c) restarting workers to pick up the new mappings. Fixed means: the `/settings` or `/Scanning` page lets the operator add a new RootFolder under any drive/share prefix, the system prompts for or auto-discovers which workers can access it, and `WorkerShareMappings` (or its `StorageRootResolutions` successor per `Core/Path/path.feature.md`) is updated without requiring a worker restart. A worker whose share mappings are updated picks up the new drive on its next continuous-scan tick.
...
29. **`ScanJobs.Progress` reflects real per-file completion within the active phase, not a per-phase constant.** Phases that expose a `ProbedFiles / FilesNeedingProbe` ratio (SizeSurvey, Probing) interpolate within the phase's reserved range using that ratio: `SizeSurvey` occupies 10-30%, `Probing` occupies 90-100%. Phases without a per-file ratio (`Walking`, `Reconciling`, `Completing`) fall back to the phase-floor constant (30 / 50 / 100). Producer-side: `_StartProgressHeartbeat` writes `Progress` through a single `_ComputeRealProgress()` helper rather than mirroring `self.ScanProgress` verbatim, so the operator-visible percentage advances continuously across the long phases instead of jumping in fixed steps. Verifiable: trigger a scan, poll `SELECT Phase, Progress, ProbedFiles, FilesNeedingProbe FROM ScanJobs WHERE Id=<id>` at 5s intervals; while `Phase='SizeSurvey'` and `FilesNeedingProbe > 0`, observe `Progress` advance from 10 toward 30 in step with `ProbedFiles / FilesNeedingProbe`; same shape 90 -> 100 during `Phase='Probing'`. Owns the producer side of the /Activity scan-progress bar smoothness; closes the "Progress stuck at 20% for entire SizeSurvey" UX complaint.

30. **New-subtree-first walk ordering (recursive scans).** For a recursive `PerformScan(<root>)`, level-1 subdirs on disk that have zero matching MediaFiles rows in the DB (`NewSubtrees = disk_L1 - db_L1`) are walked before subdirs the DB knows about. DB is source of truth (`SELECT DISTINCT LOWER(split_part(RelativePath, '/', 1)) FROM MediaFiles WHERE StorageRootId=?` under the RootFolder's RelativePath prefix). Level-1 only -- deeper reordering (new season inside known show) is out of scope. Order-only change: `ScanJobs.NewFiles/UpdatedFiles/DeletedFiles/SkippedFiles` unchanged vs pre-feature. INFO log line before walk: `Scan for <root>: N new subtree(s) prioritized: [names]` or `Scan for <root>: no new subtrees`. Excluded dirs (via `FileManagerService.ShouldExcludeDirectory`) neither classified as new nor walked. Verifiable: `Tests/Contract/TestScanNewSubtreesFirst.py` (9 tests, live DB + tmp fixture). Implementation: `FileScanningRepository.GetKnownLevel1SubdirNames` + `FileScanningBusinessService._SortNewSubtreesFirst` slotted between walk and canonical conversion in `PerformScan`. Owns operator UX complaint: freshly-dropped folder gets probed before multi-minute re-walk of existing library. Closed by directive `.claude/directives/closed/2026-07-28-scan-new-subtrees-first.md`.
...
- All 59,127 `MediaFiles.FilePath` rows are Windows drive-letter style. Linux
  ...
  insert path-translated duplicates (criterion 21 + `Core/Path/path.feature.md`).
