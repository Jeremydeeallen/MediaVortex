...
|---|---|---|---|---|---|
| S1 | `ST1 -> ST2` (start -> survey) | `StartScan` writes `ScanJobs.(Id UUID, Status='Running', Phase='SizeSurvey', WorkerName, RootFolderPath)` | `ScanJobs` row visible | `_RunSizeSurvey` reads, walks `rootfolderpath` via `os.scandir` | `SELECT Phase, WorkerName FROM ScanJobs WHERE Id=<id>` -- `Phase='SizeSurvey'`, `WorkerName` set |
...
| S3 | `ST4 -> transcode.ST2` (probe handoff) | Inline probe pass triggered by the scan worker per `FileScanning.feature.md C28` | `MediaFiles.(Resolution, AudioCodec, VideoBitrateKbps, ...)` populated; `LoudnessAnalysisService.MeasureAndPersist` writes `SourceIntegratedLufs`, `SourceLoudnessRangeLU`, `SourceTruePeakDbtp`, `LoudnessMeasuredAt` | `transcode.flow.md` Stage 2 (PROBE) treats this as already done | `SELECT Resolution, LoudnessMeasuredAt FROM MediaFiles WHERE Id IN (<rows scanned this run>)` -- non-NULL within scan wall-clock |
| S4 | `ST5 -> ST6` (reconcile -> rootfolder stats) | `ReconcileWithDisk` final DELETE of unmatched DB rows | `MediaFiles` row count for this rootfolder reflects disk truth | `_UpdateRootFolderStats` recomputes `RootFolders.(LastScannedDate, TotalSizeGB)` | `SELECT LastScannedDate, TotalSizeGB FROM RootFolders WHERE Id=<root>` matches NOW() and the post-scan SUM(SizeMB)/1024 |
