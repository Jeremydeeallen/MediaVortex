...
| ST3 | Walk filesystem | `FileScanningBusinessService._WalkRootFolder` | `os.walk` (or platform-equivalent) over `rootfolderpath`; updates `ScanJobs.CurrentDirectory` per directory enter; counts each candidate via `TotalFiles += 1`. For recursive scans, walk output is reordered new-first via `_SortNewSubtreesFirst` -- files under level-1 subdirs unknown to `MediaFiles` (DB-authoritative diff) reach `ProcessSingleMediaFile` ahead of files under known subdirs. Reconcile counters unchanged (order-only). |
...
| S6 | ST3 classify (new-subtree DB read) | `FileScanningRepository.GetKnownLevel1SubdirNames(StorageRootId, RootFolderRelPath)` -- `SELECT DISTINCT LOWER(split_part(RelativePath, '/', 1))` under prefix | `Set[str]` (lowercased level-1 subdir names) | `_SortNewSubtreesFirst` diffs against `os.scandir(LocalRootPath)` disk set; result drives stable `sorted()` on walker output | `Tests/Contract/TestScanNewSubtreesFirst.py` |
