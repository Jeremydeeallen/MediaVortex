...
C39. DEMUCS_FAILURE_SENTINEL. When `PreEncodeAudioPipeline.Run` catches an exception (Demucs crash, GPU driver fault, cuda OOM, venv-missing-module, etc.), the pipeline MUST return a sentinel dict `{DemucsFailed: True, DemucsFailureReason: '<ExceptionType>: <first 200 chars>', DemucsPremixPath: None, VocalsRmsDbfs: None}` so callers can detect the failure and proceed with Track-0-only emit rather than raising. Verifiable: `Tests/Contract/TestDemucsFailureSentinel.py` covers the three sentinel paths (downmix crash, isolate crash, reason cap). Attempt-level persistence of the failure signal is out of scope after `dialog-boost-marker-unify` (2026-08-22); a future directive may add typed columns (`TranscodeAttempts.DemucsFailed BOOL`, `DemucsFailureReason TEXT`) if operator diagnostics need SQL surface.
