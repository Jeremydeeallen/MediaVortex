...
C1. `AudioVertical.Evaluate` returns `Compliant=True` iff `MediaFiles.HasDialogBoostTrack=TRUE`. `HasDialogBoostTrack` is derived from `TranscodeAttempts.DialogBoostEmitted BOOL` (the canonical per-attempt marker, written by `AudioPreEncodeFacade.PersistMeta`) for the latest successful attempt, written by `TranscodedOutputPlacement` post-loudnorm (writer-owns-cascade). See `dialog-boost-marker-unify` for the column + rationale. Untranscoded sources return `(False, 'no_dialog_boost')`. Every encoded output ships >=2 audio streams per kept language with one carrying the `Dialog Boost` `Label` marker.
...
C17. Each `EmitTracks` entry carries `Channels` derived per-stream from `AudioStreamProbe.Probe` (`Stream['channels']`), NOT from the per-file `MediaFile.AudioChannels` scalar. Per-file scalar is a lie for multi-stream sources (e.g. eng 5.1(side) + fre 2.0); per-stream truth is what the emitter's codec-family + aformat guards must gate on. `-ac:N` is NOT emitted -- source channel count flows through the aformat filter chain when needed (see C31); redundant `-ac` on a track with aformat double-declares. Verifiable: `Tests/Contract/TestOpusMultichannelPerStream.py` synthesizes a 5.1+2.0 mixed source + asserts the 5.1 output block carries `-mapping_family:a:N 1` while the stereo output block omits it.
...
C23. `EmitTracks` carries Label, TargetLufs, TargetLra, Channels, Codec, Bitrate, SampleRateHz, BitDepth, LanguageFilter, IsDefaultTrack. `AudioNormalizationConfig` additionally carries EnableSpeechLanguageDetection, LanguageDefault, PreVerticalReNormalizePolicy.
...
C31. OPUS_MULTICHANNEL. When Track 0 codec is Opus AND the SOURCE STREAM's channel count > 2 (per-stream, from `AudioStreamProbe.Probe['channels']`; NOT per-file `MediaFile.AudioChannels`), emitter prepends `aformat=channel_layouts=5.1|7.1` to that stream's filter chain (normalizes side-layout DTS/AC3 into Opus-compatible rear-layout 5.1) AND appends `-mapping_family:a:{OutputIndex} 1` to that stream's codec args (Opus surround channel-family selector). Guard fires per-stream: a source with eng 5.1(side) + fre 2.0 emits mapping_family on the eng output block only. Without either, libopus rejects 6+ channel input with ffmpeg rc 4294967274 (Windows) / rc 234 (Linux) + stderr `Invalid channel layout 5.1(side) for specified mapping family -1`. Historic damage this closes: BUG-0087 (per-file scalar defended the wrong abstraction; multi-language 5.1+2.0 sources crashed 28/31 attempts 2026-08-07 12:00 UTC). Verifiable: `Tests/Contract/TestOpusMultichannelPerStream.py` per-stream synthetic assertion; live encode of a multi-language 5.1+2.0 source (e.g. Vida S02E04) succeeds with ffprobe showing opus 5.1 on eng output + opus 2.0 on fre output.
...
C33. NO_FORCED_DOWNMIX_AT_GLOBAL_SCOPE. Historic-damage criterion; retained as historical marker. The `MaxAudioChannels` column that gated downmix at global scope was deleted 2026-08-07 (`bug-0087-followup-maxaudiochannels-delete`); the compliance-gate check that used it was removed earlier in `transcode-flow-canonical` (C11). Modern pipeline emits per-stream channel counts via `AudioStreamProbe.Probe['channels']` (see C31); no global cap exists. Historic damage from the 2026 `MaxAudioChannels=2 + -b:a 96k` window (BUG-0072) is the residue this criterion once guarded against and now cannot recur.
...
C39. DEMUCS_FAILURE_RAISES_TO_D13. When `PreEncodeAudioPipeline.Run` encounters a Demucs failure (daemon crash, timeout, model load, ffmpeg subprocess exit non-zero, cuda OOM, venv-missing-module, etc.), the pipeline MUST raise the exception to the caller. `AudioPreEncodeFacade.Prepare` does NOT swallow. `JobProcessor.Process` catches `(DemucsDaemonUnavailableError, RuntimeError)` at the pre-encode call site and routes via `transcode.D13` partial-completion: builds w/ `AudioSlot=Copy` Plan override, executes one ffmpeg invocation, attempt lands `Success=TRUE` w/ `DispositionReason='PartialSuccess_AudioSlotCopied'` + `AudioFix` follow-up `TranscodeQueue` row auto-enqueued. Fallback ffmpeg failure fails the attempt (no second fallback -- both-slots-Copy defeats transcode). Silent Track-0-only degrade forbidden. Verifiable: `Tests/Contract/TestPreEncodeFailureRoutesToD13.py` covers pre-encode raise -> D13 fallback -> AudioFix follow-up round-trip; `Tests/Contract/TestAudioPipelineNoSilentFallback.py` greps `Features/AudioNormalization/Services/{PreEncodeAudioPipeline,AudioPreEncodeFacade}.py` for `except Exception` around Demucs invocation -- zero hits.
...
C42. PARALLEL_PREENCODE_ORCHESTRATION. `PreEncodeAudioPipeline.Run` MUST run `SourceMeasure` in a named daemon thread (`PreEncodeSourceMeasure-<JobId>`) concurrent with the `Downmix → Demucs → Premix → LoudnormMeasure` chain on the caller thread. Both paths use the same `DemucsService` instance and produce byte-identical loudnorm outputs vs a sequential predecessor because the ordering swap is orchestration-only, not math. Progress rows continue to advance in `TranscodeProgress`: source-side thread writes `PassType='SourceMeasure'`; chain writes `PassType IN ('Downmix','Demucs','Premix','LoudnormMeasure')` -- no PK collision between the two writers, and stuck-detect per `transcode.flow.md` D12 continues to observe fresh progress from both. Failure propagation: exception in either path is joined + re-raised to caller per C39 (JobProcessor routes via `transcode.D13`). Peer thread is joined normally on failure (no SIGTERM/subprocess-kill plumbing); the rare-case waste of ~30s of peer work is accepted vs the plumbing cost. Verifiable: `Tests/Contract/TestPreEncodePipelineParallel.py` (5/5) covers success-path field propagation, thread-naming, wall-time proof (parallel < sum-of-serial), and both failure-propagation modes. Measured wall reduction on I9 for a 66-min fast-group Dialog Boost job: ~53s (302s baseline → 249s parallel; median of 3 pre + 2 post samples). Motivation: SourceMeasure and Downmix are independent (both read source only, no shared output) so sequential execution was pure serialization tax on every Dialog Boost job (a mandatory-per-file cost). Historic reference: attempt 59900 (Project Runway S19E09, MediaFileId 691200, Version 156dccaf).
