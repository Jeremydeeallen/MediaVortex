...
ST2. **Pre-encode audio (Demucs).** `AudioPreEncodeFacade.Prepare(FfmpegPath, InputPath, JobId, ProgressReporter)` runs when `MediaFile.AudioCompliant IS NOT TRUE` (compliance-driven gate; audiocompliant files stream-copy audio and skip Demucs entirely). Substeps + progress emit per `PreEncodeAudioPipeline.Run`: (a0) **Input classification** -- `SourceAudioTrackSelector.SelectTrueSourceStreams(ffprobe_streams)` filters prior MediaVortex Dialog Boost tracks (identified by `tags.handler_name` startswith `Dialog Boost` OR `tags.title == 'Dialog Boost'`) before `_SelectPreferredAudioIndex` picks the Demucs input stream. When the source is a prior `-mv.mp4` whose sole audio track IS a Dialog Boost (unrecoverable input), the selector raises `PriorBoostSourceError` which propagates up through `AudioPreEncodeFacade.Prepare` and `JobProcessor.Process`; the attempt fails loud with that error message rather than silently running Demucs on prior-boost audio. Fresh sources pass through unchanged. Then (a) `SourceMeasure` 0->100 via streaming ffmpeg loudnorm on source; (b) `Downmix` 0->100 stereo WAV extract; (c) `Demucs` 0->100 vocal isolation, per-tqdm-tick percent from daemon stderr (>= 1 Hz on real workloads); (d) `Premix` 0->100 boosted-vocals + attenuated-instrumental mix; (e) `LoudnormMeasure` 0->100 via streaming ffmpeg loudnorm on premix. Each substep invokes `ProgressReporter(Phase, Percent, Info)`, which writes a `TranscodeProgress` row with `CurrentPhase=<Phase>` + `ProgressPercent=<pct>` -- `/Activity` reads these to render Phase + Progress cells during pre-encode. Returns `{DemucsPremixPath, VocalsRmsDbfs, PremixMeasuredI, PremixMeasuredLra, PremixMeasuredTp, PremixMeasuredThresh, ScratchDir}`. Failure raises `DemucsDaemonUnavailableError`, `PriorBoostSourceError`, or `RuntimeError` to caller; `JobProcessor.Process` routes Demucs failures via `transcode.D13` (AudioSlot=Copy Plan override + `AudioFix` follow-up); `PriorBoostSourceError` is a fail-loud attempt failure (no fallback -- source is unrecoverable).

ST3. **Command build.** `Strategy.BuildCommand` forwards Context (containing the ST2 premix keys) wholesale to `CommandComposer.Build(MediaFile, Job, Context)`. The composer derives a `Plan` via `PlanFactory.FromComplianceState(MediaFile)` (per transcode.flow.md D2). When `AudioCompliant=FALSE` the AudioOp is `Reencode` and `AudioSlot.Emit('Reencode', MediaFile, Context)` invokes `SourceAudioTrackSelector.SelectTrueSourceStreams(SourceStreams)` on the ffprobe stream list before calling `AudioFilterEmitter.EmitTracks(MediaFile, Policy, AudioStreams=FilteredStreams, DemucsPremixPath=..., VocalsRmsDbfs=..., PremixMeasured...=...)`. The selector runs at both consumer sites (ST2 (a0) and here) with deterministic input; identical results either place. The emitter hoists `_BuildDialogBoostBlock` ABOVE the per-stream loop (per-file arity) then iterates `AudioStreams` emitting one `_BuildOriginalBlock` per kept language stream. When `AudioCompliant=TRUE` the AudioOp is `Copy` and `AudioSlot.Emit('Copy', ...)` emits `-map 0:a? -c:a copy` with no Demucs input and no selector call (stream-copy passes source audio through unchanged). The composer emits all `-i` inputs (source + `AudioEmission.InputArgs` premix) BEFORE any `-map` args (ffmpeg parser is order-sensitive).
...
|---|---|---|---|---|---|
| S1 | ST1 -> ST2 | `JobProcessor.Process` | `(FfmpegPath: str, InputPath: str, JobId: int, ProgressReporter: callable)` | `AudioPreEncodeFacade.Prepare` returns dict with DemucsPremixPath + VocalsRmsDbfs + PremixMeasured* + ScratchDir + SourceMeasured* on success; raises `DemucsDaemonUnavailableError` or `RuntimeError` on Demucs failure (daemon crash / timeout / model load / ffmpeg subproc exit non-zero). `JobProcessor.Process` catches the raise and routes via `transcode.D13` partial-completion: builds w/ AudioSlot=Copy Plan override, executes one ffmpeg invocation, attempt lands `Success=TRUE` w/ `DispositionReason='PartialSuccess_AudioSlotCopied'` + `AudioFix` follow-up TranscodeQueue row. Return value is None only when Mode not in `_AUDIO_EMIT_MODES` or InputPath empty. | `Tests/Contract/TestAudioPreEncodeFacade.py` + `Tests/Contract/TestPreEncodeFailureRoutesToD13.py` (pre-encode raise -> D13 fallback round-trip) |
...
| S8 | ST2 (a0) input classification -> ST2 (a-e) + ST3 emit | `SourceAudioTrackSelector.SelectTrueSourceStreams(ffprobe_streams)` -- returns filtered `list[dict]` OR raises `PriorBoostSourceError` when input non-empty AND every stream carries `tags.handler_name.startswith('Dialog Boost')` OR `tags.title == 'Dialog Boost'` | Two consumer sites (deterministic filter, identical results): (a) `PreEncodeAudioPipeline._SelectPreferredAudioIndex` picks Demucs input from filtered list only -- prior-boost tracks never chosen; (b) `AudioSlot._EmitReencode` filters `SourceStreams` before invoking `EmitTracks` -- Original blocks never emitted for prior-boost input tracks. Fail-loud: `PriorBoostSourceError` propagates to `JobProcessor.Process` -> attempt fails with the error message (no fallback; source unrecoverable). | `Tests/Contract/TestSourceAudioTrackSelector.py`; I9 smoke transcode of `-mv.mp4` expects `TranscodeAttempts.Success=FALSE, ErrorMessage LIKE '%PriorBoostSourceError%'` |
