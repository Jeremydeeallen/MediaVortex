# ====================================================================
# MEDIAVORTEX: TRANSCODE, SCALE, & VALIDATION
# ====================================================================
...
# --- STEP 1b: EXTRACT NATIVE BITRATE & VALIDATE (FAIL-FAST WITH EXPLICIT CONTAINER FALLBACK) ---
Write-Host "`n------------------------------------------------------------" -ForegroundColor Yellow
...
if ($TargetWidth -gt 0 -and $TargetWidth -lt 1920) {
    # Scaling down to 720p: Target a lean budget based on 30% of source density
    $CalcBitrate = [math]::Round(($SourceBitrate * 0.30) / 1000)
    
    # Strict boundaries to force a low storage footprint for 720p
    ...
    # Keeping native 1080p resolution
...
$FFmpegArgs += @("-c:v", "av1_nvenc", "-preset", "p7", "-tune", "hq", "-multipass", "fullres", "-rc", "vbr", "-b:v", "${CalcBitrate}k", "-maxrate:v", "${CalcMaxRate}k", "-bufsize:v", "${CalcBufSize}k", "-rc-lookahead", "20", "-bf", "4", "-b_ref_mode", "middle", "-temporal-aq", "1", "-spatial-aq", "1", "-c:a", "eac3", "-b:a", "256k", "-af", "loudnorm=I=-23:LRA=15.00:TP=-2:measured_I=-21.10:measured_LRA=15.00:measured_TP=-1.10:measured_thresh=-32.10:linear=true", "-pix_fmt", "p010le", "-f", "mp4", "-movflags", "+faststart", "-metadata", "comment=Transcoded by MediaVortex", "-y", $OutputFile)
...
7.64GB file transcoded down to 595MB
VMAF score: 92.924652