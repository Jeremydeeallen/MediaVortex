...
PROBE_ARGS = ['ffmpeg', '-hide_banner', '-loglevel', 'error', '-f', 'lavfi', '-i', 'color=black:s=320x240:d=0.2', '-c:v', 'av1_nvenc', '-f', 'null', '-']
PROBE_TIMEOUT_SEC = 20


def _ProbeNvenc(SshTarget):
    try:
...
def Main():
    Parser = argparse.ArgumentParser(description='Reconcile Workers.nvenccapable via SSH ffmpeg probe on bare-metal Linux hosts.')
    ...
    Capable = _ProbeNvenc(SshTarget)
    print(f'  probe on {SshTarget}: av1_nvenc={Capable}')
