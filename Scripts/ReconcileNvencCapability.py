...
def Main():
    ...
    for Container in Containers:
        ...
        if not WorkerName:
            ...
            continue
        Capable = _ProbeNvencInContainer(SshTarget, Container)
        StoredRows = Db.ExecuteQuery(
            'SELECT nvenccapable FROM Workers WHERE WorkerName = %s', (WorkerName,)
        )
        ...
        if Args.dry_run:
            ...
            continue
        Db.ExecuteNonQuery(
            'UPDATE Workers SET nvenccapable = %s WHERE WorkerName = %s', (Capable, WorkerName)
        )
