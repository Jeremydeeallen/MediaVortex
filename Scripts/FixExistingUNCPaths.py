#!/usr/bin/env python3
"""
Fix Existing UNC Paths Script

This script finds and fixes UNC paths that were already created in the database.
It converts them back to drive letter paths.

Usage:
    py Scripts/FixExistingUNCPaths.py [--dry-run] [--verbose]
"""
...
class ExistingUNCPathFixer:
    """Fixes existing UNC paths in the database."""
    ...
    def ConvertUNCPathToDrive(self, UNCPath: str) -> str:
        ...
        try:
            # Convert \\allen\xxx\ to Z:\
            if UNCPath.startswith('\\\\allen\\xxx\\'):
                return UNCPath.replace('\\\\allen\\xxx\\', 'Z:\\')
            elif UNCPath.startswith('\\\\allen\\'):
                return UNCPath.replace('\\\\allen\\', 'Z:\\')
            
            # Add more UNC mappings as needed
            return UNCPath
            
        except Exception as e:
            LoggingService.LogException("Error converting UNC path", e, 'ConvertUNCPathToDrive', 'ExistingUNCPathFixer')
            return UNCPath
    ...
    def FixMediaFiles(self) -> int:
        ...
        try:
            ...
            LoggingService.LogException("Error fixing MediaFiles", e, 'FixMediaFiles', 'ExistingUNCPathFixer')
    ...
    def FixRootFolders(self) -> int:
        ...
        try:
            ...
            LoggingService.LogException("Error fixing RootFolders", e, 'FixRootFolders', 'ExistingUNCPathFixer')
    ...
    def FixTemporaryFilePaths(self) -> int:
        ...
        try:
            ...
            LoggingService.LogException("Error fixing TemporaryFilePaths", e, 'FixTemporaryFilePaths', 'ExistingUNCPathFixer')
    ...
    def Run(self) -> dict:
        ...
        try:
            print("🔍 Finding and fixing existing UNC paths...")
            ...
            LoggingService.LogException("Error in UNC path fixing process", e, 'Run', 'ExistingUNCPathFixer')
            return {'Success': False, 'Error': str(e)}


def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Fix existing UNC paths in database')
    parser.add_argument('--dry-run', action='store_true', help='Preview changes without making them')
    parser.add_argument('--verbose', action='store_true', help='Show detailed logging')
    
    args = parser.parse_args()
    
    print("🚀 Existing UNC Path Fixer")
    print("=" * 50)
    
    if args.dry_run:
        print("🔍 Running in DRY RUN mode - no changes will be made")
    
    Fixer = ExistingUNCPathFixer(DryRun=args.dry_run, Verbose=args.verbose)
    Result = Fixer.Run()
