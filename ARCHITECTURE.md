...
| Profiles | Exists but does not own `EffectiveProfileResolver` (lives in Compliance). | Rehome `EffectiveProfileResolver` from `Features/Compliance/` to `Features/Profiles/`. | Tied to compliance refactor |
| MediaProbe | Substantial vertical (controller + business service + repository + view model) but lacks a top-level `*.feature.md` with Cross-Vertical Contract. | Create `Features/MediaProbe/media-probe.feature.md` with Cross-Vertical Contract section. | New directive |
| Activity | Has sub-feature docs (`activity-dashboard-improvements.feature.md`, `active-jobs-filter-sort.feature.md`) + `activity-dashboard.flow.md` but no top-level vertical contract. | Create `Features/Activity/activity.feature.md` with Cross-Vertical Contract section. | New directive |
