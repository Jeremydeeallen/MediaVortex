...
from Services.JellyfinService import JellyfinService
...
class OptimizationViewModel:
    ...
    def _CheckMitigation(self, FileName: str, FilePath: str, OperationType: str, Reason: str,
        ...
        When the file was found via fuzzy match (different filename), auto-rescans with ffprobe
        to ensure DB has current metadata before checking mitigation.
        Always returns current file data (CurrentCodec, CurrentAudioCodec, CurrentContainer)
        so the UI can show what's actually on disk vs what Jellyfin logged."""
        try:
            ...
            if OperationType == "Transcode":
                if Reason == "legacy_codec":
                    ...
                    # Mitigated only if the file has been processed AND no burn-in subtitle codecs remain.
                    # Empty SubtitleFormats does NOT mean mitigated — it means we don't have subtitle data.
