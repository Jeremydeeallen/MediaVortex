# directive: path-class-perfection | # see path.C26
import re
from pathlib import Path as PyPath
from unittest.mock import MagicMock

from Features.FileReplacement.FileReplacementBusinessService import FileReplacementBusinessService
from Features.FileReplacement.TranscodedOutputPlacement import TranscodedOutputPlacement
from Core.Path.Worker import Worker


_FR_DIR = PyPath(__file__).resolve().parent.parent.parent / "Features" / "FileReplacement"
_OS_PATH_VAR_RX = re.compile(r"(?i)os\.path\.(exists|isfile|isdir|getsize|getmtime|dirname|basename|join|split|splitext)\s*\(\s*\w*(?:path|filepath)\w*")


# directive: path-class-perfection | # see path.C26
def test_no_pathstorage_import_in_filereplacement():
    """Regression-guard: zero Core.PathStorage references (the legacy v1 module deleted by path-perfect-implementation)."""
    Offenders = []
    for File in _FR_DIR.rglob("*.py"):
        Src = File.read_text(encoding="utf-8")
        if "Core.PathStorage" in Src:
            Offenders.append(str(File))
    assert not Offenders, f"FileReplacement reintroduced Core.PathStorage: {Offenders}"


# directive: path-class-perfection | # see path.C26
def test_no_os_path_on_path_variable_in_filereplacement():
    """R6 regression-guard: no os.path.<op>(<path-named var>) callsites."""
    Offenders = []
    for File in _FR_DIR.rglob("*.py"):
        Src = File.read_text(encoding="utf-8")
        for LineNo, Line in enumerate(Src.splitlines(), start=1):
            if _OS_PATH_VAR_RX.search(Line):
                Offenders.append(f"{File}:{LineNo}: {Line.strip()}")
    assert not Offenders, f"FileReplacement has os.path on path-named vars: {Offenders}"


# directive: path-class-perfection | # see path.C26
def test_business_service_worker_constructor_injected():
    """C26: __init__ sets self._Worker via constructor-injection (default = Worker.Current()); no _StorageRoots field after AC #2 cache decomposition."""
    Mock = MagicMock()
    S = FileReplacementBusinessService(DatabaseManagerInstance=Mock)
    assert isinstance(S._Worker, Worker)
    assert not hasattr(S, '_StorageRoots')


# directive: path-class-perfection | # see path.C26
def test_business_service_worker_constructor_accepts_test_injection():
    """C26: a test can pass an explicit Worker; the singleton lookup is bypassed."""
    MockWk = MagicMock(spec=Worker)
    S = FileReplacementBusinessService(DatabaseManagerInstance=MagicMock(), worker=MockWk)
    assert S._Worker is MockWk


# directive: path-class-perfection | # see path.C26
def test_transcoded_output_placement_worker_constructor_injected():
    """C26: TranscodedOutputPlacement has the same constructor-injection contract."""
    Mock = MagicMock()
    T = TranscodedOutputPlacement(DatabaseManagerInstance=Mock)
    assert isinstance(T._Worker, Worker)
    assert not hasattr(T, '_StorageRoots')
