...
- **WorkerService** -- Unified worker: transcoding, VMAF quality testing, and file scanning. Each worker reads capability flags (TranscodeEnabled, QualityTestEnabled, ScanEnabled) and status (Online/Draining/Offline) from its Workers table row.
...
- **PostgreSQL 16** on dedicated LXC container
...
- **Database encoding must be UTF-8** -- media file paths contain non-ASCII characters
...
- No emojis in .md files (plain text only)
- Prefer editing existing files over creating new ones
- All operations should be idempotent
- For features with a user-facing surface, write the flow doc first, then feature doc + criteria
- Every in-progress feature doc has a `### Progress` checklist under `## Status`
