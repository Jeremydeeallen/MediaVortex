...
- `deploy-baremetal-worker.py` / `deploy-windows-worker.py` post-restart verification line prints "version=<sha>" alongside FFmpeg path and heartbeat age
...
3. **`deploy-baremetal-worker.py` stamps VERSION.** `StepStampVersion` writes the dev workstation's `git rev-parse HEAD` into `/opt/mediavortex/src/VERSION` and `/opt/mediavortex/src/BUILD_INFO` on every deploy.
...
- [x] Doc sweep: `WorkerService/WorkerService.flow.md` Version section rewritten; `deploy/worker-deploy-baremetal.flow.md` post-deploy verify SELECT gained Version
...
| `deploy/deploy-baremetal-worker.py` | `StepStampVersion` writes VERSION + BUILD_INFO on target; `StepVerify` asserts `Workers.Version == stamped SHA` per worker |
...
| `deploy/worker-deploy-baremetal.flow.md` | Post-deploy verify SELECT gained Version column |
