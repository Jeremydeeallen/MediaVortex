...
19. **[BUG 2026-05-12 - FIXED 2026-05-13]** The Linux-worker scp step and Windows `StepScpRepo()` now use `.deployignore` exclusion patterns instead of blind `scp -r`. Linux deploy uses `deploy/SyncSource.py` (tar-over-ssh with pattern filtering). Windows deploy uses `shutil.copytree` with the same `.deployignore` patterns into a temp directory before scp. Additive: new source files are included by default. Targeted: `.git`, `venv`, `__pycache__`, `Tests`, `specs`, `FFmpegMaster`, `cache`, `archive_*`, test artifacts, and other non-build files are excluded. Verifiable: `py deploy/SyncSource.py root@10.0.0.42 /tmp/mediavortex-build --dry-run` shows only build-relevant files; `du -sh /tmp/mediavortex-build` on the LXC after copy is bounded; documented in `deploy/worker-deploy.flow.md` step 1.

20. **[BUG 2026-05-13]** `deploy/worker-deploy.flow.md` is incomplete as an operational runbook. The "Build and Deploy Pipeline" section ends at `docker compose up -d` + optional verification, but does not document: (a) post-deploy health checks confirming FFmpeg/FFprobe paths resolve inside the container (`Workers.FFmpegPath` is non-NULL and points at a real binary), (b) the full "container started -> worker is fully operational" sequence (DB registration, share mapping registration, capability loading, crash recovery), (c) troubleshooting steps when FFmpeg path resolution fails (e.g., `docker exec` to verify binary presence, checking Workers row), (d) what additional operator actions are needed after first deploy vs code-only redeploy. Verifiable when fixed: an operator who has never deployed before can follow the flow doc from zero to verified-operational without referring to source code or asking questions. The doc should have parity with `deploy/windows-worker.flow.md` in terms of post-deploy verification and troubleshooting coverage. Look first: `deploy/worker-deploy.flow.md` -- compare its post-deploy coverage to `deploy/windows-worker.flow.md`. Fix with `/t`.
