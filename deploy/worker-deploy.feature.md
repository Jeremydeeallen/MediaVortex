...
14. **[BUG-0064] Deploy story is clean: I9 local-only, remote workers independent, one SOLID script.** The deploy surface enforces a hard split between local I9 services and remote worker hosts, and consolidates the remote path into a single entry script.
    - **I9 local services have no deploy path.** WebService and the local WorkerService start from their respective venvs (`WebService/venv/` for WebService, repo-root `venv/` for the worker). The start command brings WebService online FIRST, then the WorkerService. Before starting the local WorkerService, the command MUST detect any running WorkerService process on I9 and stop it; verify zero instances; then start (one parent + child = two processes max). No SyncSource, Task Scheduler, Docker, or `deploy-*-worker.py` invocation participates in the I9 path.
    - **Remote-worker deploys are independent.** Deploying any one remote worker host (larry/wakko/dot/future) MUST NOT read state from, depend on, or affect any other remote worker. No fleet orchestration step blocks host A on host B's heartbeat or build success.
    - **Single SOLID deploy script.** One operator-facing entry script handles every remote shape via per-shape strategy classes (LXC-Docker, bare-metal-Docker, Windows-SMB). Constructor-DI; the entry script owns only CLI parsing, inventory lookup, and verification polling; each strategy owns its own bring-up. No copy-paste between OS branches, no shared mutable state, no shell-pipeline-shaped Python. The current `deploy-linux-worker.py` + `deploy-windows-worker.py` pair is collapsed into the new single entry.
    - **Documentation purged of stale references.** Every reference to "deploying I9" is removed from `deploy/bringup.md`, `deploy/worker-deploy-windows.flow.md`, and any other deploy/runbook doc. Verifiable: `grep -rniE 'deploy.*i9|i9.*deploy' deploy/` returns 0 hits.
