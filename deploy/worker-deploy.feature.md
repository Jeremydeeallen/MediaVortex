...
- [x] 10. Deployed dot (2026-05-16). Manual host bootstrap (Docker CE 29.5.0, nfs-common, 3 NFS mounts from Wakko's pattern, /staging, /opt/mediavortex). Then `py deploy/deploy-linux-worker.py dot` end-to-end -- 4 `dot-worker-1..4` rows Online with non-NULL FFmpegPath, heartbeat < 5s. Proving ground PASS for criteria 1-6, 9-10.
