...
13. **[BUG-0008] I9 NFS writes never return EINVAL.** FFmpeg invocations on I9-2024 -- both true transcodes (libsvtav1) and Remux/Quick (stream copy) -- complete their `open()` of the output `.mp4.inprogress` file without intermittent `Invalid argument` failures. Verifiable: across any 100 consecutive `TranscodeAttempts` rows where `ClaimedBy='I9-2024'`, none have `Success=false` with `ErrorMessage` matching `return code 4294967274` AND `TranscodeDurationSeconds=0`. Linux workers writing to the same porky NFS export already meet this bar with zero failures; the bar for I9 is equality with that baseline.
