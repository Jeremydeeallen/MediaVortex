...
### --- BUG-0039 mint+close 2026-06-02 ---

### [BUG-0039 - RESOLVED 2026-06-02] Workers attempt jobs for MediaFiles entries whose source file no longer exists on disk | resolved: 2026-06-02 (source-existence pre-flight + FFprobeFailureCount guard + mount validation (ProcessTranscodeQueueService.py:370/539/813))
**Date:** 2026-05-08
...
### --- BUG-0041 mint+close 2026-06-02 ---

### [BUG-0041 - RESOLVED 2026-06-02] QueryDatabase.py truncates long text columns at 60 chars -- error messages unreadable | resolved: 2026-06-02 (print_table no longer truncates (QueryDatabase.py:48))
**Date:** 2026-05-13
...
### --- BUG-0032 close 2026-06-02 (14 files recovered, 137 orphans flagged) ---

### [BUG-0032 - RESOLVED 2026-06-02] Stale .orig files from remux re-queue loop -- 205 files affected, 14 at risk of data loss | resolved: 2026-06-02 (directive: bug-0032-orig-recovery)
**Date:** 2026-05-14
...
### [BUG-0068] AudioFilterEmitter STRATEGY_REVIEW bypass leaves audio bitrate uncontrolled -- fallback chain hides the real principle | resolved: 2026-06-27
**Date:** 2026-06-23 -> Closed 2026-06-25 by directive `audio-pipeline-fail-loud` (Phase D wiring + contract tests 3/3). Tracker bookkeeping completed 2026-06-27. | **Area:** transcode-emit + audio-policy | **Resolution:** `Features/AudioNormalization/audio-normalization.feature.md` C26 (PROFILE_CEILING holds unconditionally) + C27 (POLICY_OBSERVABILITY persistence) + C28 (NO_SILENT_PATH AST contract test) + C29 (OPERATOR_VISIBLE_FAILURE). `AudioFilterEmitter.EmitTracks` `STRATEGY_REVIEW: continue` branch retired -- REVIEW classifier output now routes through `AudioDispositionResolver.ResolveForTrack` via `_BuildReviewFallbackBlock` which clamps emitted bitrate to `Profile.TargetAudioKbps` or raises `AudioPolicyUnresolvedError`. `TranscodeShape` literal `['-c:a', 'copy']` extend removed; codec args for the empty-Blocks branch come from `EAC3OrPassthroughCodecPolicy.Decide`. `TranscodeAudioPolicyVerdicts` table + `TranscodeAttempts.AudioPolicyResolved` column added for verdict persistence. `AudioPipelineFailHandler.HandleUnresolved` writes Faulted state on `AudioPolicyUnresolvedError`. Verified by `Tests/Contract/TestAudioBitratePolicyHonorsCeiling.py` 3/3, `Tests/Contract/TestAudioPolicyPersistenceAndFailHandler.py` 3/3, `Tests/Contract/TestAudioPipelineNoSilentFallback.py` AST walk, `Tests/Contract/TestAudioOperatorVisibleFailure.py` 2/2 -- 53/53 audio cluster contract tests PASS.

**What broke:** When source loudness was too extreme to gain safely (e.g. -37 LUFS), `AudioStrategyClassifier` returned `STRATEGY_REVIEW` for every track. `AudioFilterEmitter.EmitTracks` then `continue`d on every track and returned 0 blocks. `TranscodeShape.Build` hit the fallback branch and emitted `-c:a copy` -- source bitrate (160kbps in the failure case) survived into the output. The post-transcode compliance gate then flipped `ComplianceGateFailed: bitrate:160>134` and the file was stuck in `Transcode` bucket forever.

The pipeline had TWO fallbacks stacked: (1) Strategy classifier silently demoted to REVIEW when gain budget exceeded with no signal to downstream that the loudnorm path was abandoned; (2) TranscodeShape silently swapped in `-c:a copy` when blocks were empty with no awareness that source bitrate was above the profile's audio ceiling. Both hid the real situation. The principle: every output track has an explicit policy decision that is either (a) honored (loudnorm + ceiling clamp), or (b) the job FAILS LOUD with a "this file cannot be made compliant under the current profile" error visible to the operator.
