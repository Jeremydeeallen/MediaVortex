...
- BUG-0067 | active | file-replacement | TranscodedOutputPlacement orphan-on-failure -- when _UpdateMediaFilesAfterReplacement fails (unique-key collision, re-probe error, etc.) the rename is left in place + Success=True returned + warning logged; scanner ingests the orphan as a duplicate row; next encode attempt fails identically; loop snowballs. Replace fallback with explicit rollback + loud failure | 2026-06-23
- BUG-0068 | active | transcode-emit | AudioFilterEmitter STRATEGY_REVIEW bypass + TranscodeShape bare -c:a copy fallback -- when classifier returns REVIEW for every track, 0 blocks emit, TranscodeShape falls back to stream-copy and source bitrate survives past Profile.TargetAudioKbps ceiling; two stacked silent fallbacks hide the real issue. Must honor ceiling unconditionally or fail loud with logged reason; reshapes audio-bitrate-clamp from common-case-only to all-paths | 2026-06-23
