...
@TranscodeQueueBlueprint.route('/Statistics', methods=['GET'])
def GetStatistics():
    """Get queue statistics."""
    try:
        LoggingService.LogFunctionEntry("GetStatistics", "TranscodeQueueController")
        
        # Create ViewModel instance
        viewModel = TranscodeQueueViewModel()
        
        # Get statistics
        result = viewModel.GetQueueStatistics()
        
        if result.get("Success", False):
            LoggingService.LogInfo("Retrieved queue statistics", "TranscodeQueueController", "GetStatistics")
            return jsonify(result)
        else:
            LoggingService.LogError(f"Failed to get statistics: {result.get('ErrorMessage', 'Unknown error')}", "TranscodeQueueController", "GetStatistics")
            return jsonify(result), 500
            
    except Exception as e:
        errorMsg = f"Exception getting statistics: {str(e)}"
        LoggingService.LogException(errorMsg, e, "TranscodeQueueController", "GetStatistics")
        return jsonify({"Success": False, "ErrorMessage": errorMsg}), 500


@TranscodeQueueBlueprint.route('/Filter', methods=['POST'])
def FilterQueue():
    """Filter queue items by status and/or priority."""
    try:
        LoggingService.LogFunctionEntry("FilterQueue", "TranscodeQueueController")
        ...
        if status and not isinstance(status, str):
            errorMsg = "Status must be a string"
            LoggingService.LogError(errorMsg, "TranscodeQueueController", "FilterQueue")
            return jsonify({"Success": False, "ErrorMessage": errorMsg}), 400
        ...
        loadResult = viewModel.LoadQueueItems()
        if not loadResult.get("Success", False):
            return jsonify(loadResult), 500
        
        # Filter items
        filteredItems = viewModel.FilterQueueItems(status, priority)
        ...
        LoggingService.LogInfo(f"Filtered queue to {len(filteredItems)} items", "TranscodeQueueController", "FilterQueue")
        return jsonify(result)
        
    except Exception as e:
        errorMsg = f"Exception filtering queue: {str(e)}"
        LoggingService.LogException(errorMsg, e, "TranscodeQueueController", "FilterQueue")
        return jsonify({"Success": False, "ErrorMessage": errorMsg}), 500
