...
- [x] `PostTranscodeDispositionService.RecordComplianceGateFailure` flips a previously-committed `Replace` disposition to `NoReplace`/`ComplianceGateFailed` and writes the cascade reason to `TranscodeAttempts.ErrorMessage`. TFP cleanup runs automatically via `_CommitDisposition` (NoReplace is in the existing cleanup set).
